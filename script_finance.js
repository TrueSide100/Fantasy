...
// FIX(23): `playerText` may be either a plain string (legacy behavior — every call site
// before this fix, and still fine for callers that don't need the ledger below) or an array
// of {index, text} objects, one per player message in the caller's window, tagged with that
// message's absolute position in the chat array. Passing the array form additionally enables
// the already-used-confirmation ledger below; passing a string skips it entirely, unchanged
// from before.
//
// THE BUG THIS LEDGER CLOSES: updatePanel() (script_chatroom.js) widened the confirmation
// window to the last 10 messages (FIX(10)) so a real spend confirmed 1-2 turns late wouldn't
// get wrongly blocked. But every turn re-runs this guard over that same window — so a single
// "I paid 100 gold" sits there as valid-looking evidence for up to ~5 turns. If the background
// model mistakenly proposes ANOTHER decrease in that time, this guard had no way to tell that
// evidence apart from a fresh confirmation, and let it through every time. The ledger records,
// per currency, which message index already grounded a decrease, so the same line can never
// validate a second one — while a genuinely new confirming message still can.
function guardCurrencyDecreases(data, panel, playerText, recentLogText){
  if(!data || !data.categories) return data;
  const indexedMessages = Array.isArray(playerText) ? playerText : null;
  // FIX(5): quoted speech stripped before the confirmation regex ever sees the text.
  const text = stripQuotedSpeech(indexedMessages ? indexedMessages.map(m=>m.text||'').join('\n') : (playerText || ''));
  const groundingHaystack = String(recentLogText || '') + ' ' + text;
  const knownCurrencies = knownCurrencyKeyNames(panel);
  const ledger = (indexedMessages && panel) ? (panel.currencyGroundLedger || (panel.currencyGroundLedger = {})) : null;
  ...
  for(const [catName, catUpdate] of Object.entries(data.categories)){
    ...
    for(const [k, v] of Object.entries(catUpdate.kv)){
      ...
      const rx = decreaseConfirmRegexForKey(k);
      let confirmed, confirmingIndices = [];
      if(indexedMessages){
        for(const m of indexedMessages){
          if(rx.test(stripQuotedSpeech(m.text || ''))) confirmingIndices.push(m.index);
        }
        confirmed = confirmingIndices.length > 0;
      } else {
        confirmed = rx.test(text);
      }
      if(!confirmed){
      ...
      // FIX(23): every confirming message for this currency in the window has already been
      // used to ground an earlier decrease — this isn't fresh evidence, it's the same "I paid
      // 100 gold" line still sitting in the window validating a second/third/fourth drop.
      // Block it exactly like having no confirmation at all.
      let freshIndices = confirmingIndices;
      if(ledger){
        const normKey = normalizeCurrencyKey(k);
        const used = ledger[normKey] || [];
        freshIndices = confirmingIndices.filter(i => !used.includes(i));
        if(freshIndices.length === 0){
          console.warn(`[money guard] blocked ${catName}.${k} decrease (${oldNum} -> ${newNum}) — the only "<verb> ... ${k}" confirmation(s) in the recent window were already used to ground an earlier decrease`);
          delete catUpdate.kv[k];
          continue;
        }
      }
      ...
      // FIX(23): this decrease cleared every check and is about to be applied — mark its
      // confirming message(s) as used so they can't ground another decrease later while
      // they're still sitting inside the window. Capped at 20 per currency; only indices
      // still inside the live 10-message window are ever checked against, so older entries
      // are dead weight, not a correctness risk.
      if(ledger){
        const normKey = normalizeCurrencyKey(k);
        const used = ledger[normKey] || (ledger[normKey] = []);
        for(const i of freshIndices) if(!used.includes(i)) used.push(i);
        while(used.length > 20) used.shift();
      }
    }
    if(Object.keys(catUpdate.kv).length === 0) delete catUpdate.kv;
  }
