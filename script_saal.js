...
// Shared by both the whole-lore gate and the per-entry proximity check inside seedSkillsFromLore
// below — kept as one definition so the two checks can never drift out of sync with each other.
// Bug fix: several alternatives below used to have no trailing word-boundary, so they matched as
// a bare PREFIX of completely unrelated real words — "skill" inside "skillet", "mana" inside
// "manager"/"manatee"/"manacle", "organ" inside "organic"/"organize"/"organization" — meaning
// ordinary sentences ("she picked up a skillet", "the shop manager") wrongly satisfied both the
// whole-lore gate and the per-entry grounding check, defeating the point of either. Fixed with
// either a trailing \b or a negative lookahead blocking the specific unrelated words. Also fixed
// the opposite bug: "art\b"/"form\b"/"style\b" required the word to end exactly there, so they
// never matched their own plurals ("arts", "forms", "styles") even though those are completely
// normal phrasing ("various magical arts", "unique fighting styles"). "train" is a deliberate,
// narrower tradeoff: matched only with an -ed/-ing/-er/-ee/-s suffix, never bare, since the bare
// word is exactly as likely to mean the vehicle as the verb, and a vehicle mention on its own
// should never be able to satisfy this gate; the residual gap is a bare present-tense "I train
// hard" alone won't match this word (something else in the same sentence almost always will).
const ABILITY_CONTEXT_RE = /\b(power|abilit|skills?(?!et)|techniqu|jutsu|spell|magic|affinit|innate|inborn|bloodline|born with|gift|talent|master|train(?:ed|ing|er|ee|s)\b|learn|studied|cultivat|system|status\s*window|interface|organs?(?!ic|iz)|mutat|mutant|curse|blessing|supernatural|superhuman|enchant|sorcery|wizard|elemental|psychic|telekin|energy|chakra|mana(?!ge|cle|tee)|\bki\b|aura|arts?\b|forms?\b|styles?\b)/i;
async function seedSkillsFromLore(world){
  if(!world.lore || !world.lore.trim()) return;
  // Hard code-level gate, checked BEFORE even calling the model: the gibberish filter further
  // below (looksLikeGibberishSkillLabel) only catches labels that don't look like real words at
  // all — it does nothing against a real dictionary word or name that has no actual connection
  // to a power/skill (e.g. "venda" slipping through because it happens to be a real word, even
  // though nothing in the setup text describes it as an ability). The actual signal that
  // matters is whether the lore text contains ANY language that indicates a power/skill/system
  // is being described in the first place. If none of these context words appear anywhere in
  // the lore at all, there is nothing legitimate for this pass to find — skip the model call
  // entirely and add nothing, rather than trusting the model's own judgment on borderline text.
  if(!ABILITY_CONTEXT_RE.test(world.lore)){
    console.warn('[skill guard] skipped Skills & Abilities lore-seed entirely — no power/skill/system language found anywhere in the world\'s setup text');
    return;
  }
  ...
  try{
    ...
    for(const [name, upd] of Object.entries(data.categories)){
      ...
      const cleaned = upd.list_add.filter(label => {
        if(looksLikeGibberishSkillLabel(label)){
          console.warn(`[skill guard] blocked lore-seeded entry "${label}" — doesn't look like a real word/name`);
          return false;
        }
        if(!entryGroundedInLore(label, world.lore, ABILITY_CONTEXT_RE)){
          console.warn(`[skill guard] blocked lore-seeded entry "${label}" — sits in the lore text with no power/skill/system language anywhere near it`);
          return false;
        }
        return true;
      });
      if(cleaned.length) skillsOnly.categories[name] = { list_add: cleaned };
    }
...
// Hard code-level backstop for seedSkillsFromLore below: even with the prompt above telling the
// model what to skip, an LLM can still slip and hand back a meaningless label built from real
// setup text (a placeholder, a scrambled/random string, a lone fragment with no real word in
// it) — this catches that deterministically rather than trusting the model's own judgment.
// Strips the (Innate Power)/(Affinity) tag first (that's a legitimate suffix, not part of the
// label being judged), then checks whether the label is actually made of real-looking words:
// a word needs a vowel (a/e/i/o/u, including y as a vowel stand-in) and at least 2 letters to
// count as "real" on its own — bare consonant clusters ("bbn", "kkmx") or single letters ("T")
// don't. An entry is rejected if it has no real words at all, or if real words make up less
// than half of it — a short label needs every word pulling weight; a long one can tolerate an
// invented proper noun or two (fantasy jutsu/spell names often coin a word) without being
// thrown out just for that.
function looksLikeGibberishSkillLabel(label){
  const base = skillBaseName(label).toLowerCase();
  const words = base.match(/[a-z]+/g) || [];
  if(!words.length) return true; // nothing alphabetic at all — reject
  const isRealish = w => w.length >= 2 && /[aeiouy]/.test(w);
  const realCount = words.filter(isRealish).length;
  return realCount === 0 || realCount < words.length / 2;
}
// Second layer, catching what looksLikeGibberishSkillLabel structurally cannot: a real word or
// name (like "venda") that simply has nothing to do with any power — it reads fine letter-wise
// but isn't actually describing an ability anywhere in the text. Rather than judge the word in
// isolation, check whether it sits near actual power/skill/system language IN THE LORE ITSELF:
// find where the entry's own name appears in the raw lore text and look at a generous window of
// surrounding text for the same context vocabulary the top-level ABILITY_CONTEXT_RE gate checks
// for. A wide window (300 chars each side) tolerates a full descriptive sentence around the
// name without being so wide it stops meaning anything. If the entry's name can't be found in
// the lore text at all (the model paraphrased/combined wording rather than quoting it), this is
// deliberately permissive and lets it through — punishing a reasonable paraphrase would cause
// more harm than the rare invented-name-with-no-textual-anchor case it might miss.
function entryGroundedInLore(label, loreText, contextRe){
  const base = skillBaseName(label).toLowerCase().trim();
  if(!base) return false;
  const lore = String(loreText||'').toLowerCase();
  const idx = lore.indexOf(base);
  if(idx === -1) return true; // can't locate it verbatim — don't penalize a paraphrased name
  const start = Math.max(0, idx - 300);
  const end = Math.min(lore.length, idx + base.length + 300);
  return contextRe.test(lore.slice(start, end));
}
...
// Intervening-word window widened from 3 to 6: natural phrasing like "I spent the rest of the
// evening training" or "I sat down and quietly began studying" pushes the actual verb past 3
// words from "I" fairly easily, and each of those was silently failing to match before — same
// gap the {0,3} widening (from an original 0) was already fixing, just not far enough.
// Verb list widened past stud/train/practic (already present) to also cover perform, hone,
// grind, meditat(e), spar, and exercis(e) — more of the words a player naturally reaches for
// when describing focused effort on a skill, so the same practice-phrase bar isn't effectively
// gated on guessing one specific verb.
const PRACTICE_TRIGGER_RE = /\bi\b(?:'|’)?(?:ve|m)?(?:\s+(?!(?:not|never|n't|refuse\w*|avoid\w*|stop\w*|quit\w*|skip\w*|can'?t|won'?t|wouldn'?t|don'?t|didn'?t)\b)\w+){0,6}\s*\b(?:stud(?:y|ied|ying)|open(?:ed|ing)?|read(?:ing)?|learn(?:ed|t|ing)?|practic(?:e|ed|ing)|train(?:ed|ing)?|drill(?:ed|ing)?|rehears(?:e|ed|ing)|work(?:ed|ing)?\s*on|review(?:ed|ing)?|go(?:es)?\s*over|perform(?:ed|ing|s)?|hone(?:d|ing)?|grind(?:ed|ing)?|meditat(?:e|ed|ing)|spar(?:red|ring)?|exercis(?:e|ed|ing)?)\b/i;
