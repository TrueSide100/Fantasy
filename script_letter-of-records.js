...
const EQUIP_SLOT_RULES = [
  ...
  { slot:'wrist-upper-left', icon:'⬜', label:'Left wrist — inner', group:'Wrists', kws:[] },
  { slot:'wrist-lower-left', icon:'⬜', label:'Left wrist — outer', group:'Wrists', kws:[] },
  { slot:'wrist-front-right', icon:'⬜', label:'Right wrist — inner', group:'Wrists', kws:[] },
  { slot:'wrist-back-right', icon:'⬜', label:'Right wrist — outer', group:'Wrists', kws:[] },
...
// Groups EQUIP_SLOT_RULES into { group, rules } sections in array order (one entry per
// group, in the order groups first appear) — shared by both the compact icon grid and the
// full legend so "which slots sit together" is defined once, not duplicated.
function groupEquipSlotRules(){
  const sections = [];
  let cur = null;
  for(const rule of EQUIP_SLOT_RULES){
    if(!cur || cur.group !== rule.group){ cur = { group: rule.group, rules: [] }; sections.push(cur); }
    cur.rules.push(rule);
  }
  return sections;
}
...
function renderEquipCompassHtml(items){
  ...
  const sections = groupEquipSlotRules();
  // Compact icon grid: plain flat list, same as before — every slot in EQUIP_SLOT_RULES
  // gets one cell, in array order. No grouping/styling here; grouping is legend-only (below).
  ...
  // Legend is grouped by body region (see `group` on each rule) so related slots sit
  // together under a clear heading, with generous space between one region and the next —
  // rather than one long undifferentiated list.
  const legendRows = sections.map(sec => {
    const rows = sec.rules.map(rule=>{
      const item = bySlot[rule.slot];
      return `<div class="equip-legend-row">
        <span class="equip-legend-icon${item ? ' is-filled' : ''}">${rule.icon}</span>
        <div class="equip-legend-text">
          <div class="equip-legend-label${item ? ' is-filled' : ''}">${escapeHtml(rule.label)}</div>
          <div class="equip-legend-name${item ? '' : ' is-empty'}">${item ? escapeHtml(item.name) : 'Empty'}</div>
        </div>
      </div>`;
    }).join('');
    return `<div class="equip-legend-section">
      <div class="equip-legend-section-title">${escapeHtml(sec.group)}</div>
      ${rows}
    </div>`;
  }).join('');
