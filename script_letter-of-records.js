...
function renderEquipCompassHtml(items){
  ...
  // "Also carried" (accessories that count as on-body but don't map to a specific slot)
  // used to render here as a second, plain-pill chip list — visually inconsistent with the
  // photo-backed Inventory cards above, and fully redundant with them since every one of
  // these items is already shown there. Suppressed; accessories are still computed above
  // (bySlot/legend logic doesn't depend on this) but no longer get their own block.
