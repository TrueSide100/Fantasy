...
function renderEquipCompassHtml(items){
  ...
  // "Equipment Equipped" (the grouped BODY/HEAD & FACE/etc legend rows below the hub) and
  // "Also carried" (accessories that count as on-body but don't map to a specific slot) have
  // both been removed per request — the equip-box (hub + slot grid) above is the only thing
  // this renders now. legendRows/accessories are still computed above since computeEquipSlots
  // is shared plumbing (bySlot/groupEquipSlotRules), just no longer output here.
  void legendRows;
...
// ---------- inventory item long-press action sheet (Equip/Unequip, Change Photo) ----------
// Long-pressing an inventory card now opens a bottom-up action sheet for that item — its full
// name, current status, an Equip/Unequip toggle, and a Change Photo action (which is where the
// photo picker moved to; see setInventoryItemPhoto) — instead of jumping straight to the photo
// picker as before. Deliberately scoped to cards carrying data-photo-editable (set only on the
// main Letter of Records view — see renderPanelHtml) so this never competes with the dedicated
// Inventory page's own long-press-to-drag gesture.
(function(){
  ...
  const cancelBtn = document.getElementById('itemActionCancelBtn');
  ...
  if(equipBtn) equipBtn.addEventListener('click', async ()=>{
    ...
    const onBody = isOnBodyStatus(activeStatus);
    closeSheet();
    await setInventoryItemStatus(state.chattingId, activeItemId, onBody ? null : 'Equipped');
  });
