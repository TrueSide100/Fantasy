...
// ---------- [TL-3b] FALLBACK — Current Day update the background model missed ----------
// The background "letter of records" pass (updatePanel) reads the recent log with its own
// AI model and proposes whatever category changes it notices — Current Day included. That
// works fine when the log states a skip plainly ("the next morning"), but a model can still
// miss one it has to infer rather than read literally (e.g. a player saying "after 30 days
// passed" and the very next line narrating a scheduled event as happening "today" — the day
// number itself is never spelled out, so the model has to connect two separate cues). When
// that happens there's no error, nothing throws, so showBackgroundWarning never fires either —
// the panel just silently keeps its old Current Day forever with zero visible sign anything
// went wrong. This runs BEFORE guardTimelineDay, purely as a backstop: if the recent log
// text itself supports a time-skip (the exact same deterministic phrase-scan guardTimelineDay
// already trusts) and the AI's proposed update didn't touch Current Day at all, synthesize one
// here from that same phrase-scan instead of leaving it untouched. guardTimelineDay then runs
// as normal afterward and still clamps this synthesized value to a due Scheduled Events entry
// exactly the same way it would clamp an AI-proposed one — this only fills in a missing
// proposal, it never bypasses the guard that follows it.
// A single narrated time-skip is very often restated more than once in the very same message —
// a line of narration ("you spend three days...") AND a line of in-character dialogue repeating
// the same span ("Three days of cowering...", said by an NPC) both describe ONE event, not two.
// maxAllowedDaySkip (above) is a SUM across every match it finds — correct for its job as a
// ceiling checked against an AI-proposed delta over a wide, multi-turn window, where genuinely
// separate skips really can add up. ensureTimelineDayFallback's job is different: it has no
// AI-proposed number to validate, so it has to manufacture the actual amount to add from a
// single two-message turn — and for that, taking the LARGEST individual mention in the turn
// (not the sum) is the correct model: a turn represents one coherent skip, stated once or
// restated for flavor, never several independent skips stacked in the same exchange.
function maxSingleDaySkip(text){
  text = String(text || '');
  let best = 0;
  let m;
  VARIABLE_SKIP_RE.lastIndex = 0;
  while((m = VARIABLE_SKIP_RE.exec(text))){
    if(isCountdownContext(text, m.index, m.index + m[0].length)) continue;
    const days = skipPhraseToDays(m[1], m[2]);
    if(days > best) best = days;
  }
  FIXED_SKIP_RE.lastIndex = 0;
  while((m = FIXED_SKIP_RE.exec(text))){
    const days = /month/i.test(m[1]) ? 30 : /week/i.test(m[1]) ? 7 : 1;
    if(days > best) best = days;
  }
  return best;
}
function ensureTimelineDayFallback(data, panel, recentLogText, worldId){
  if(!data || !panel || !panel.categories) return data;
  let timelineCatName = null, oldNum = null;
  for(const [catName, cat] of Object.entries(panel.categories)){
    ...
    for(const [k, v] of Object.entries(cat.data)){
      ...
      if(!isNaN(n)){ timelineCatName = catName; oldNum = n; }
    }
  }
  if(timelineCatName == null || oldNum == null) return data; // Timeline untracked — nothing to fall back on
  data.categories = data.categories || {};
  const existingUpdate = data.categories[timelineCatName];
  const alreadyProposed = existingUpdate && existingUpdate.kv &&
    Object.keys(existingUpdate.kv).some(k => CURRENT_DAY_KEY_RE.test(k));
  if(alreadyProposed) return data; // AI already proposed a change — let guardTimelineDay validate it as-is
  // Player-only input is a single short message, not restated narration — the sum-based
  // maxAllowedDaySkip (used as a ceiling elsewhere) is the right fit here too, since a player
  // deliberately writing "wait 3 days, then 4 more" really does mean 7, and there's no
  // restatement/backstory risk left to guard against once the story's own reply text is out
  // of the picture.
  const allowedSkip = maxAllowedDaySkip(recentLogText);
  if(allowedSkip <= 0) return data; // nothing in the log actually supports a skip — correctly nothing to add
  data.categories[timelineCatName] = existingUpdate || {};
  data.categories[timelineCatName].kv = data.categories[timelineCatName].kv || {};
  data.categories[timelineCatName].kv['Current Day'] = String(oldNum + allowedSkip);
  showGuardWarning(worldId, `Timeline — the background update missed a day change the log supports; advancing Current Day by ${allowedSkip} as a fallback (${oldNum} → ${oldNum + allowedSkip}).`);
  return data;
}
  ...
  // old behavior of silently skipping every removal check the moment currentDay was null.
  {
    for(const [catName, catUpdate] of Object.entries(data.categories)){
      ...
      catUpdate.list_remove = catUpdate.list_remove.filter(entry=>{
        ...
        if(isSchedCat){
          showGuardWarning(worldId, `Scheduled Events — blocked removing "${entry}": the story can't clear a scheduled event on its own — only the player can, with the ✕ on the entry itself.`);
          return false;
        }
        const parsed = parseDayEntry(entry);
        if(!parsed){
...
// ---------- [TL-7b] per-entry delete for Scheduled Events (the ✕ on a chip) ----------
// Manual, player-initiated removal — deliberately separate from guardScheduledEvents above,
// which only ever polices AI-proposed list_remove/list_add and has nothing to do with this
// path. That guard's whole point is to stop the STORY from silently clearing a due date;
// it says nothing about the player themselves choosing to delete an entry from their own
// sheet on purpose. Mirrors deleteSkillEntryById in script_saal.js exactly: matched by the
// entry's own permanent hidden id (never by position or text, so a duplicate-looking "Day
// N — ..." line can't cause the wrong one to go), and only mutates the in-memory panel —
// the caller (the panelContent click handler in script_letter_of_records.js) is responsible
// for savePanel + repaint afterward.
function deleteScheduledEventById(panel, id){
  if(!panel || !panel.categories || !id) return false;
  const schedKey = findExistingKey(panel.categories, 'Scheduled Events');
  const schedCat = schedKey ? panel.categories[schedKey] : null;
  if(!schedCat || schedCat.type !== 'list' || !Array.isArray(schedCat.ids)) return false;
  const idx = schedCat.ids.indexOf(id);
  if(idx === -1) return false;
  // Same ids/data drift guard as deleteSkillEntryById — refuse rather than risk splicing
  // the wrong entry if the two parallel arrays are ever out of sync.
  if(!Array.isArray(schedCat.data) || idx >= schedCat.data.length){
    console.warn(`[scheduled event delete] ids/data out of sync for Scheduled Events (id index ${idx}, data length ${schedCat.data ? schedCat.data.length : 'n/a'}) — refusing to delete to avoid removing the wrong entry`);
    return false;
  }
  schedCat.ids.splice(idx, 1);
  schedCat.data.splice(idx, 1);
  return true;
}
