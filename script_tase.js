/* script_tase.js — Timeline and Scheduled Events: the deterministic guards
   ...
   script_fieb.js (Finances/Inventory/Equip Box). This file is the Timeline + Scheduled
   Events half of script_tsal.js — split again so it can be read and edited independently
   of the Skills & Abilities + Learning half (script_saal.js). Depends on
   globals from index.html's inline Section 1 (load that file first); load order
   relative to script_chatroom.js, script_fieb.js, and script_saal.js doesn't
   matter — all share one global scope.
   ...
   Sections 3 (SKILLS & ABILITIES) and 4 (LEARNING) — the permanent-record and
   in-progress-percentage halves of the original script_tsal.js — now live in
   script_saal.js.
