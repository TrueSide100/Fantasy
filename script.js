...
async function paintPanel(){
  ...
  els.panelContent.innerHTML = renderPanelHtml(panel, { pendingMerge: recPendingMerge });
  ...
  if(!recPendingMerge){
    ...
    // LONG_PRESS_MS arms it (small haptic buzz + a lift animation) as the signal that the
    // next move will carry it, rather than scroll the sheet. Moving too far before that
    // timer fires cancels the press outright so an ordinary scroll/tap is never mistaken
    // for a drag. Built on Pointer Events (not HTML5 drag-and-drop) since HTML5 DnD doesn't
    // fire from a finger touch on most phones/tablets — Pointer Events unify mouse, touch,
    // and pen so the same code handles a mouse drag on desktop and a finger drag on a phone
    // identically.
    const chips = Array.from(els.panelContent.querySelectorAll('.panel-chip[data-id]'));
    let drag = null; // { id, label, chip, armed, startX, startY, ghost, timer }
    const LONG_PRESS_MS = 380;      // hold time before a press arms into a draggable pickup
    const PRESS_CANCEL_PX = 10;     // movement past this before arming cancels the press

    const clearDragVisuals = ()=>{
      chips.forEach(c=>{ c.classList.remove('is-drop-target'); c.classList.remove('is-dragging'); c.classList.remove('is-pressing'); c.classList.remove('is-armed'); });
    ...
    chips.forEach(chip=>{
      ...
      // these pointer handlers get a chance to treat it as a long-press.
      ...
      chip.addEventListener('pointermove', (e)=>{
        ...
        if(!drag.armed){
          ...
          // person is scrolling or just brushed the chip, not trying to pick it up.
          if(Math.hypot(dx, dy) >= PRESS_CANCEL_PX) clearDragVisuals();
          return;
        }
        if(!drag.ghost){
          const ghost = document.createElement('div');
          // Reuses the .panel-chip class (plus .panel-chip-ghost for the floating/elevated
          // bits — see index.html) so the ghost is a faithful copy of the real item box:
          // same font, border, padding, colour. It genuinely looks like the tag itself being
          // picked up and carried, not a stray line of text.
...
els.closePanelBtn.onclick = ()=>{ els.panelModal.style.display = 'none'; recPendingMerge = null; };
els.panelModal.onclick = (e)=>{ if(e.target===els.panelModal){ els.panelModal.style.display='none'; recPendingMerge = null; } };
...
function renderPanelHtml(panel, opts){
  ...
  const sections = orderedCategoryEntries(panel).map(([name, cat])=>{
    ...
    if(cat.type==='kv'){
      ...
      // Drag-to-merge is Inventory-only: every chip gets a data-id so a long-press-drag can
      // target the exact entry regardless of duplicate wording, plus a confirm bar once a
      // valid pair is dropped together. There's no separate "merge mode" to switch on
      // anymore — long-pressing any item is always how a drag starts. Every other list
      // category (Skills, Milestones, Scheduled Events, etc.) renders exactly as before,
      // untouched.
      const invIds = (isInventoryCat && Array.isArray(cat.ids)) ? cat.ids : [];
      const mergeBarHtml = (isInventoryCat && invPendingMerge)
        ? `<div class="panel-merge-bar">
             <div class="panel-chip panel-merge-chip">
               <input type="text" class="panel-merge-input" placeholder="New item name" value="${escapeHtml(invPendingMerge.a.label + ' + ' + invPendingMerge.b.label)}" />
        ...
        : '';
      const mergeHintHtml = (isInventoryCat && !invPendingMerge && cat.data && cat.data.length > 1)
