...
async function reconcilePanelFromMemory(world, memory, panel){
  ...
  try{
    ...
    guardItemMerge(data, panel, '', memory);
...
// Summoned alongside the System's reply (typing "system bro") — two semi-square tiles
// rendered INSIDE that same reply's bubble (Scheduled Events / Inventory), each one
// expanding its own tool in place when tapped. This replaces the old setup where
// "add a schedule event ..." was a second free-text chat command a player could type
// mid-story (and which wrote straight to the letter of records with zero review) — that
// text trigger is gone now; the only way to add a scheduled event is through this
// deliberate, structured form, reachable only after explicitly summoning the System.
// Only ever shown on the latest system message (same rule the regenerate/continue
// actions already follow), and both tools always paint from live storage so they can
// never show or act on a stale snapshot.
async function attachSystemToolsPanel(wrap){
  ...
  box.innerHTML = `
    <div class="sys-tools">
      ...
      <button type="button" class="sys-tool-tile" data-tool="inv">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>
        <span>Inventory</span>
      </button>
    </div>
    <div class="sys-tool-body" data-body="sched"></div>
    <div class="sys-tool-body" data-body="inv"></div>
  `;
  bubble.appendChild(box);

  const schedBody = box.querySelector('[data-body="sched"]');
  const invBody = box.querySelector('[data-body="inv"]');
  const tiles = box.querySelectorAll('.sys-tool-tile');
  const closeAll = ()=>{
    schedBody.classList.remove('open'); invBody.classList.remove('open');
    tiles.forEach(t=>t.classList.remove('active'));
  };

  // ---- Inventory tap-to-remove (same delete logic as before, just relocated into the bubble) ----
  // NEW FEATURE: merging two items into a new one, either by tapping both in sequence or by
  // dragging one onto the other. This state has to live out here, not inside paintInv itself,
  // because paintInv rewrites invBody.innerHTML from scratch on every repaint — anything kept
  // inside it would reset every time the panel redraws (e.g. right after a background story
  // turn touches the sheet while merge mode is open).
  let mergeMode = false;
  let mergeSelection = []; // up to two { id, label } entries queued for merging
  let dragSourceId = null;
  const paintInv = async ()=>{
    const panel = await getPanel(worldId);
    const inv = panel.categories['Inventory'];
    const items = (inv && inv.type==='list') ? inv.data : [];
    const ids = (inv && inv.type==='list' && Array.isArray(inv.ids)) ? inv.ids : [];
    // If the sheet changed underneath an open merge selection (an item got used up, renamed,
    // or removed by a story turn while this panel was open), drop any selected id that no
    // longer exists rather than let a stale selection try to merge something that's gone.
    mergeSelection = mergeSelection.filter(sel => ids.includes(sel.id));
    const headerHtml = `<div class="sys-inv-title-row" style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
      <div class="sys-inv-title">Inventory${mergeMode ? ' — tap or drag two items to merge' : ' — tap to remove'}</div>
      <button type="button" class="sys-inv-merge-toggle" style="font-size:12px;padding:2px 8px;border-radius:6px;">${mergeMode ? 'Cancel' : 'Merge'}</button>
    </div>`;
    const mergeBarHtml = (mergeMode && mergeSelection.length === 2)
      ? `<div class="sys-inv-merge-bar" style="display:flex;align-items:center;gap:6px;margin:6px 0;">
           <span style="font-size:12px;white-space:nowrap;">${escapeHtml(mergeSelection[0].label)} + ${escapeHtml(mergeSelection[1].label)} →</span>
           <input type="text" class="sys-inv-merge-input" placeholder="New item name" value="${escapeHtml(mergeSelection[0].label + ' + ' + mergeSelection[1].label)}" style="flex:1;min-width:0;" />
           <button type="button" class="sys-inv-merge-confirm" style="font-size:12px;padding:2px 8px;border-radius:6px;">Merge</button>
         </div>`
      : '';
    invBody.innerHTML = headerHtml + mergeBarHtml + (items.length
      ? `<div class="sys-inv-list">${items.map((it, i)=>{
          const text = String(it);
          const dashIdx = findItemStatusDashIndex(text); // accepts en dash too — see splitItemEntry fix
          const label = dashIdx===-1 ? text : text.slice(0, dashIdx).trim();
          const status = dashIdx===-1 ? null : text.slice(dashIdx+1).trim();
          const id = ids[i] || '';
          const isSelected = mergeMode && mergeSelection.some(s => s.id === id);
          // Targeted by the item's own hidden ID (not its position or text) so an action
          // always applies to exactly the tapped/dragged entry, even with several identically-
          // worded items on the sheet, and even if the list's order or contents shift
          // underneath this panel before the action is processed.
          return `<button class="sys-inv-item" data-id="${escapeHtml(id)}" data-label="${escapeHtml(label)}" type="button" draggable="${mergeMode ? 'true' : 'false'}" style="${isSelected ? 'outline:2px solid #7c9cff;outline-offset:-2px;' : ''}">
            <span class="sys-inv-label">${escapeHtml(label)}${status ? `<span class="sys-inv-status">${escapeHtml(status)}</span>` : ''}</span>
            ${mergeMode
      ...
      : `<div class="sys-inv-empty">No items on the sheet right now.</div>`);

    const mergeToggleBtn = invBody.querySelector('.sys-inv-merge-toggle');
    ...
    // Deterministic, user-initiated merge: no AI/guard chain involved here at all, since the
    // player directly picked both ingredients and the resulting name themselves — there's
    // nothing to ground or confirm that the player hasn't already confirmed by doing this.
    const doMerge = async (idA, idB, newName)=>{
      ...
      await queueWorldOp(worldId, async ()=>{
        ...
        const idxs = [idA, idB].map(id => freshInv.ids.indexOf(id)).filter(i => i !== -1).sort((a,b)=>b-a);
        if(idxs.length !== 2) return; // one of the two selected items vanished from under this action — abort safely, remove nothing
        for(const idx of idxs){ freshInv.data.splice(idx, 1); freshInv.ids.splice(idx, 1); }
        freshInv.data.push(name);
        freshInv.ids.push(genId());
    ...
    const mergeConfirmBtn = invBody.querySelector('.sys-inv-merge-confirm');
    if(mergeConfirmBtn) mergeConfirmBtn.onclick = ()=>{
      const input = invBody.querySelector('.sys-inv-merge-input');
      const [a, b] = mergeSelection;
      if(a && b) doMerge(a.id, b.id, input ? input.value : '');
    };

    invBody.querySelectorAll('.sys-inv-item').forEach(btn=>{
      const id = btn.dataset.id;
      const label = btn.dataset.label;
      if(mergeMode){
        // ---- tap-to-select path: works identically with mouse or touch, and is the only
        // reliable option on most touchscreens, since the native HTML5 drag-and-drop events
        // below generally don't fire on mobile without extra polyfilling this app doesn't have.
        btn.onclick = ()=>{
          const already = mergeSelection.findIndex(s => s.id === id);
          if(already !== -1){ mergeSelection.splice(already, 1); paintInv(); return; }
          if(mergeSelection.length >= 2) return; // a pair is already queued — confirm or cancel it first
          mergeSelection.push({ id, label });
          paintInv();
        };
        // ---- drag-and-drop path: desktop/mouse convenience on top of the tap-to-select path
        // above, not a replacement for it.
        btn.ondragstart = (e)=>{
          dragSourceId = id;
          e.dataTransfer.effectAllowed = 'move';
          e.dataTransfer.setData('text/plain', id); // Firefox refuses to start a drag at all without data set here
        };
        btn.ondragover = (e)=>{ if(dragSourceId && dragSourceId !== id) e.preventDefault(); };
        btn.ondrop = (e)=>{
          e.preventDefault();
          const sourceId = dragSourceId || e.dataTransfer.getData('text/plain');
          dragSourceId = null;
          if(!sourceId || sourceId === id) return;
          const sourceBtn = invBody.querySelector(`.sys-inv-item[data-id="${CSS.escape(sourceId)}"]`);
          const sourceLabel = sourceBtn ? sourceBtn.dataset.label : sourceId;
          mergeSelection = [{ id: sourceId, label: sourceLabel }, { id, label }];
          paintInv();
        };
      } else {
        btn.onclick = async ()=>{
          ...
          btn.disabled = true;
          // Routed through the same per-world op queue the AI-driven letter-of-records writes
          // use (see updatePanel/resyncMemoryAndPanel) — otherwise a delete tapped while a
          // background sheet update from the last story turn is still resolving could read a
          // stale snapshot and silently overwrite this deletion, or vice versa.
          await queueWorldOp(worldId, async ()=>{
            const fresh = await getPanel(worldId);
            const freshInv = fresh.categories['Inventory'];
            if(!freshInv || !freshInv.data || !Array.isArray(freshInv.ids)) return;
            // Matched purely by ID — immune to reordering, renaming, or duplicate wording,
            // unlike the old "same text, Nth occurrence" approach this replaced.
            const at = targetId ? freshInv.ids.indexOf(targetId) : -1;
  ...
  // block with the rest of the timeline/schedule logic; only the UI glue lives here, since this
  // function also builds the Inventory tile right above and the two share one panel shell.)
...
async function savePanel(id, panel){ await kvSet('wc_panel_'+id, panel); }
...
// being ADDED to "Skills & Abilities" early; listedAbilityNames (below) stops the PLAYER's own
...
// Parsing "<item> — <status>" entries, the quantity guard for stackable items, the
// duplication-math backstop, and the equip-status/rename-bypass/discard guards.
...
// BUG FIX / NEW FEATURE (item crafting/merging): none of the action-confirmation regexes in
// this file recognized a combine/craft/merge/fuse-type action at all, so "I combine my Knife
// and Poison Bottle into a Poisoned Knife" fell through every guard below with no recognized
// verb — either blocked outright, or (worse) let the new item through while one or both
// ingredients stayed on the sheet untouched. This is the shared trigger every guard below now
// also checks for, alongside its own existing verb list, so a crafting action can confirm an
// identity change (rename), an ingredient's permanent consumption (discard), and — paired with
// guardItemMerge further down — actually prove real ingredients were spent before a freshly
// "new" item is ever trusted.
const CRAFT_TRIGGER_RE = /\b(i\s*combin(?:e|es|ed|ing)|i\s*merg(?:e|es|ed|ing)|i\s*craft(?:s|ed|ing)?|i\s*fus(?:e|es|ed|ing)|i\s*mix(?:es|ed|ing)?|i\s*forg(?:e|es|ed|ing)|i\s*assembl(?:e|es|ed|ing)|i\s*brew(?:s|ed|ing)?|i\s*synthesiz(?:e|es|ed|ing)|i\s*build(?:s|ing)?)\b/i;
function guardStackableItems(data, panel, playerText, recentLogText){
  if(!data || !data.categories) return data;
  const playerNamedAbility = playerNamedListedAbility(playerText, panel);
  const playerConfirmedUse = ITEM_USE_TRIGGER_RE.test(playerText || '') || CRAFT_TRIGGER_RE.test(playerText || '') || playerNamedAbility;
  const playerConfirmedState = ITEM_STATE_TRIGGER_RE.test(playerText || '') || playerNamedAbility;
  ...
  for(const [catName, catUpdate] of Object.entries(data.categories)){
    ...
    for(const entry of catUpdate.list_add){
      ...
      if(!oldEntry){ fixedAdd.push(entry); continue; } // brand-new stackable item, not a decrease — allow
      const oldParsed = splitItemEntry(oldEntry);
      if(oldParsed.qty == null || isNaN(oldParsed.qty)){ fixedAdd.push(entry); continue; }
      ...
      if(newQty > oldParsed.qty){
        ...
        const nearbyNumbers = extractNumbersNearKey(label, groundingHaystack);
      ...
      if(parsed.status !== null && parsed.status !== oldParsed.status){
        if(playerConfirmedState){ finalStatus = parsed.status; }
        else console.warn(`[item guard] blocked "${label}" status change ("${oldParsed.status||'none'}" -> "${parsed.status}") — no confirming action phrase in player's last message`);
      }
...
function guardDuplicationMath(data, panel, recentLogText){
  ...
  const windowText = matchLine.toLowerCase();
  ...
  for(const [catName, existingCat] of Object.entries(panel.categories)){
    ...
    for(const entry of existingCat.data){
      ...
      if(anchor.length < 4 || !windowText.includes(anchor)) continue;
...
function guardInventoryEquipStatus(data, panel, playerText){
  if(!data || !data.categories) return data;
  const playerConfirmed = ITEM_STATE_TRIGGER_RE.test(playerText || '') || CRAFT_TRIGGER_RE.test(playerText || '') || playerNamedListedAbility(playerText, panel);
  if(playerConfirmed) return data; // player explicitly did something to an item — let it through normally
  for(const [catName, catUpdate] of Object.entries(data.categories)){
    ...
    catUpdate.list_add = catUpdate.list_add.filter(entry=>{
      ...
      console.warn(`[inventory status guard] blocked "${entry}" — status change on an existing item with no confirming action phrase in player's last message`);
...
function guardInventoryRenameBypass(data, panel, playerText, recentLogText){
  if(!data || !data.categories) return data;
  const playerConfirmed = ITEM_STATE_TRIGGER_RE.test(playerText || '') || CRAFT_TRIGGER_RE.test(playerText || '') || playerNamedListedAbility(playerText, panel);
  const playerConfirmedUse = ITEM_USE_TRIGGER_RE.test(playerText || '') || CRAFT_TRIGGER_RE.test(playerText || '') || playerNamedListedAbility(playerText, panel);
  const groundingHaystack = String(recentLogText || '') + ' ' + (playerText || '');
  for(const [catName, catUpdate] of Object.entries(data.categories)){
    if(!catUpdate || !Array.isArray(catUpdate.list_add)) continue;
    const existingCat = panel && panel.categories ? panel.categories[findExistingKey(panel.categories, catName) || catName] : null;
    ...
    for(const entry of catUpdate.list_add){
      ...
      if(!playerConfirmed){
        console.warn(`[inventory rename-bypass guard] blocked "${entry}" — looks like an unpaired rename/elaboration of existing "${fuzzyOld}" with no confirming action phrase in player's last message`);
      ...
      if(parsedNew.qty != null && oldParsed.qty != null && parsedNew.qty !== oldParsed.qty){
        ...
        if(qty < oldParsed.qty && !playerConfirmedUse){
          console.warn(`[inventory rename-bypass guard] blocked quantity decrease riding along with rename of "${fuzzyOld}" (${oldParsed.qty} -> ${qty}) — no use/spend phrase in player's last message; keeping old quantity`);
          qty = oldParsed.qty;
...
function guardInventoryDiscard(data, panel, playerText){
  if(!data || !data.categories) return data;
  const playerConfirmed = DISCARD_TRIGGER_RE.test(playerText || '') || CRAFT_TRIGGER_RE.test(playerText || '') || playerNamedListedAbility(playerText, panel);
  for(const [catName, catUpdate] of Object.entries(data.categories)){
    ...
    catUpdate.list_remove = catUpdate.list_remove.filter(entry=>{
      ...
      if(playerConfirmed) return true;
      console.warn(`[inventory discard guard] blocked permanently removing "${entry}" — no discard phrase ("I threw/left/dropped/discarded/sold/...") in player's last message`);
      return false;
    });
    if(catUpdate.list_remove.length === 0) delete catUpdate.list_remove;
  }
...
// ---------- NEW FEATURE / hard code-level guard: crafting/merging two or more EXISTING items
// into one brand-new named item (e.g. "I combine my Knife and Poison Bottle into a Poisoned
// Knife"). This is the backstop for exactly the failure mode above the other guards can't catch
// on their own: when the crafted result's name doesn't happen to share words with either
// ingredient (a genuinely new, invented name — "Venomfang Blade" instead of "Poisoned Knife" —
// is exactly what a real craft SHOULD produce), guardInventoryRenameBypass's fuzzy word-overlap
// match never fires at all, so the new item looks IDENTICAL in shape to an ordinary find/buy/
// gift — nothing before this point checks that anything was actually spent to make it. Left
// alone, that's a free item mint riding on a crafting narrative, with the "spent" ingredients
// possibly never actually leaving the sheet.
//
// Scoped narrowly on purpose: this only ever activates when the player's own message actually
// contains a combine/craft/merge/fuse-type phrase, and even then it never tries to guess WHICH
// specific items were meant to be the ingredients — guessing that from freeform text isn't
// reliable enough to gate an inventory change on. It just requires proof: at least two of this
// same turn's list_remove entries must match REAL items that were actually on the sheet a
// moment ago (not merely claimed) before any accompanying item that doesn't already exist is
// trusted as a legitimate craft result. A fabricated "removal" that names nothing really owned
// proves nothing was spent, and doesn't count. No such proof, no new item — exactly like every
// other unconfirmed grant in this file, the sheet just stays as it was.
function guardItemMerge(data, panel, playerText, recentLogText){
  if(!data || !data.categories) return data;
  const craftClaimed = CRAFT_TRIGGER_RE.test(playerText || '') || CRAFT_TRIGGER_RE.test(recentLogText || '');
  if(!craftClaimed) return data; // no combine/craft/merge/fuse phrase this turn — nothing for this guard to do
  for(const [catName, catUpdate] of Object.entries(data.categories)){
    if(!catUpdate || !Array.isArray(catUpdate.list_add)) continue;
    const existingCat = panel && panel.categories ? panel.categories[findExistingKey(panel.categories, catName) || catName] : null;
    if(!existingCat || existingCat.type !== 'list') continue;
    const removeList = Array.isArray(catUpdate.list_remove) ? catUpdate.list_remove : [];
    // Only removals that actually match something that WAS on the sheet count as genuine —
    // this runs after guardInventoryDiscard/guardInventoryRenameBypass above, so by this point
    // any removal that survived has already cleared its own confirmation check too.
    const genuineRemovals = removeList.filter(r => existingCat.data.some(old => itemNameKey(old) === itemNameKey(r)));
    catUpdate.list_add = catUpdate.list_add.filter(entry=>{
      const label = itemNameKey(entry);
      const isPairedRewrite = removeList.some(r => itemNameKey(r) === label); // an exact-name correction, not a freshly-crafted item — another guard's job
      const existsAlready = existingCat.data.some(old => itemNameKey(old) === label);
      if(isPairedRewrite || existsAlready) return true;
      if(genuineRemovals.length >= 2) return true; // at least two real ingredients confirmed spent — trust the craft
      console.warn(`[item merge guard] blocked "${entry}" — a combine/craft/merge/fuse phrase was used, but fewer than 2 genuine existing items were removed this turn to back it up; likely a free item mint riding on the crafting narrative`);
      return false;
    });
    if(catUpdate.list_add.length === 0) delete catUpdate.list_add;
  }
...
async function updatePanel(world, opts){
  ...
  try{
    ...
    guardItemMerge(data, panel, lastPlayerMsg ? lastPlayerMsg.text : '', recent);
...
async function resyncMemoryAndPanel(world, chat){
  ...
  try{
    ...
    guardItemMerge(data, oldPanel, playerTextInWindow, convo);
...
function renderPanelSection(title, innerHtml){
  return `<div class="panel-section"><div class="panel-sec-title">${title}</div>${innerHtml}</div>`;
}
function renderPanelHtml(panel){
  ...
  const sections = orderedCategoryEntries(panel).map(([name, cat])=>{
    ...
    if(cat.type==='kv'){
      ...
      inner = (cat.data && cat.data.length)
        ? `<div class="panel-chips">${cat.data.map(it=>{
            ...
            const label = dashIdx === -1 ? text : text.slice(0, dashIdx).trim();
            const status = dashIdx === -1 ? null : text.slice(dashIdx+1).trim();
            return `<span class="panel-chip${isPast ? ' is-past' : ''}">${escapeHtml(label)}${status ? `<span class="panel-chip-status">${escapeHtml(status)}</span>` : ''}</span>`;
          }).join('')}</div>`
        : `<div class="panel-empty">(none yet)</div>`;
    }else{
      inner = cat.data
    ...
    return renderPanelSection(escapeHtml(name), inner);
  }).join('');
