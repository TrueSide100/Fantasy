...
function wireInventoryChipDrag(container, worldId, pendingRef, repaint){
  ...
  const doDelete = async (id, label)=>{
    ...
    await queueWorldOp(worldId, async ()=>{
      ...
      // Legacy numMap entries are no longer read after migration (see migrateLegacyInventoryIds
      // / numFromId), but clearing this one out anyway is free and keeps the map from
      // accumulating dead entries for items that no longer exist.
...
// and what doMerge/doDelete above look up fresh by ID from storage, not by screen position) is
...
function renderPanelHtml(panel, opts){
  ...
  const sections = orderedCategoryEntries(panel).filter(([name])=> opts.tabFilter ? panelTabForCategory(name) === opts.tabFilter : !SCHED_CAT_RE.test(name)).map(([name, cat])=>{
    let inner;
    let headerExtra = '';
