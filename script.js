    ...
    if(cat.type==='kv'){
      ...
      inner = mergeHintHtml + ((cat.data && cat.data.length)
        ? `<div class="${chipsClass}">${cat.data.map((it, i)=>{
            ...
            const labelHtml = `${numHtml}${escapeHtml(nameOnly)}${qtyHtml}${status ? `<span class="panel-chip-status">${escapeHtml(status)}</span>` : ''}`;
...
const CAT_INFO = [
  ...
  { test:/^inventory/i, title:'Inventory',
    ...
    use:'Tap the expand icon next to \u201cInventory\u201d on the Letter of Records to open the dedicated Inventory page. There you can long-press and drag one item onto another to merge them into a new combined item, or tap the \u2715 on any chip to permanently delete it yourself \u2014 both happen immediately and don\u2019t need the story to confirm them, since they\u2019re actions you\u2019re taking directly rather than something narrated.'
  },
