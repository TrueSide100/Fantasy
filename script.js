...
async function paintPanel(){
  ...
  if(mergeConfirmBtn) mergeConfirmBtn.onclick = ()=>{
    if(recPendingMerge) doMerge(recPendingMerge.a.id, recPendingMerge.b.id, recPendingMerge.name);
  };
  ...
  if(!recPendingMerge){
    ...
    const armDrag = ()=>{
      ...
      drag.ghost = ghost;
      // Where within the chip the finger actually grabbed it, so the clone doesn't jump to
      // some fixed corner the moment it starts following the pointer.
    ...
    chips.forEach(chip=>{
      ...
      chip.addEventListener('pointerdown', (e)=>{
        ...
        clearDragVisuals();
        drag = { id: chip.dataset.id, label: chip.dataset.label, chip, armed: false, startX: e.clientX, startY: e.clientY, offsetX: 0, offsetY: 0, ghost: null, timer: null };
      ...
      const endDrag = (e)=>{
        ...
        recPendingMerge = { a: { id: sourceId, label: sourceLabel }, b: { id: targetChip.dataset.id, label: targetChip.dataset.label }, name: autoMergeName(sourceLabel, targetChip.dataset.label) };
        paintPanel();
      };
      ...
      const mergeBarHtml = (isInventoryCat && invPendingMerge)
        ? `<div class="panel-merge-bar">
             <div class="panel-chip panel-merge-chip">
               <span class="panel-merge-name">${escapeHtml(invPendingMerge.name || (invPendingMerge.a.label + ' ' + invPendingMerge.b.label))}</span>
