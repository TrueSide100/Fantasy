...
window.addEventListener('popstate', ()=>{
  ...
  if(els.invModal.style.display === 'flex'){ els.invModal.style.display = 'none'; invPendingMerge = null; return; }
...
// ---------- shared inventory drag-to-merge wiring ----------
// Long-press-drag-to-merge (and the resulting confirm/cancel name bar) now lives ONLY on
// the dedicated, larger-size Inventory-only page (openInventoryModal below) — the main
// Letter of Records shows Inventory as plain, non-interactive chips. Kept as its own
// function (rather than inlined into paintInventoryModal) purely so the ~100 lines of
// pointer-event plumbing don't clutter the modal's own open/paint/close logic.
...
els.closeInvBtn.onclick = ()=>{ els.invModal.style.display = 'none'; invPendingMerge = null; };
els.invModal.onclick = (e)=>{ if(e.target===els.invModal){ els.invModal.style.display='none'; invPendingMerge = null; } };
...
function renderPanelHtml(panel, opts){
  ...
  const sections = orderedCategoryEntries(panel).map(([name, cat])=>{
    ...
    if(cat.type==='kv'){
      ...
      const mergeBarHtml = (mergeActive && invPendingMerge)
      ...
      inner = mergeBarHtml + mergeHintHtml + ((cat.data && cat.data.length)
        ? `<div class="${chipsClass}">${cat.data.map((it, i)=>{
            ...
            const idAttr = mergeActive ? ` data-id="${escapeHtml(invIds[i]||'')}" data-label="${escapeHtml(label)}"` : '';
