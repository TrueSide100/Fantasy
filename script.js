...
// Finds the closing brace that actually matches the first '{', by walking forward and
// tracking nesting depth (and skipping over braces that appear inside quoted strings). Using
// cleaned.lastIndexOf('}') here used to just grab the LAST '}' anywhere in the whole reply —
// if any trailing text after the real object happened to contain its own '}' (stray
// commentary, a second stray object, etc.), the slice included that extra text too, which
// JSON.parse then rejected with "Unexpected non-whitespace character after JSON" even though
// the object itself was perfectly valid. Walking depth-by-depth from the first '{' finds the
// TRUE end of that object, whatever comes after it.
...
// Decides whether a claimed ability's significant words actually match something on the
// sheet, scaled by how many words the claim has. A plain "any word overlaps" check let a claim
// like "fire dragon technique" (fire, dragon) pass just because "fire" also appears in "Mouth
// Fire-Breathing Technique" on the sheet — "dragon" was never actually checked. Rule:
//   1-3 significant words: EVERY word must be found on the sheet.
//   4+ significant words: every word except at most one must be found (one word can be
//   skipped, e.g. for minor phrasing differences), but never fewer than that.
function abilityWordsMatch(words, knownWords){
  if(!words.length) return false;
  const matched = words.filter(w => knownWords.has(w)).length;
  if(words.length <= 3) return matched === words.length;
  return matched >= words.length - 1;
}
