...
const EQUIP_SLOT_RULES = [
  { slot:'head', icon:'⛑️', label:'Head', kws:['helm','kabuto','hat','hood','headband','crown','circlet'] },
  { slot:'face', icon:'🎭', label:'Face', kws:['mask','veil','goggles','blindfold','visor'] },
  { slot:'neck', icon:'📿', label:'Neck', kws:['amulet','necklace','pendant','torc','choker'] },
  { slot:'chest', icon:'🛡️', label:'Chest', kws:['armor','cuirass','robe','coat','jacket','vest','kimono','plate','breastplate'] },
  { slot:'back', icon:'🎒', label:'Back', kws:['cloak','cape','mantle','backpack','quiver','satchel'] },
  { slot:'wrist', icon:'⛓️', label:'Wrist', kws:['bracer','wristguard','vambrace','cuff'] },
  { slot:'palm', icon:'🧤', label:'Hand', kws:['glove','gauntlet','mitten','ring'] },
  ...
  { slot:'foot', icon:'🥾', label:'Feet', kws:['boot','tabi','shoe','sandal','geta'] },
  { slot:'weapon', icon:'⚔️', label:'Weapon', kws:['katana','sword','blade','kunai','dagger','axe','spear','bow','staff','wand','tanto'] },
  { slot:'offhand', icon:'🔰', label:'Off-hand', kws:['shield','buckler','tome','orb','lantern'] },
];
// Fixed 5x5 grid position for each slot — the outer ring of a 5x5 grid, with the center 3x3
// block reserved for the hub icon. Each edge holds its 4 slots with a gap left over the
// middle of that edge (directly facing the hub) for even, breathing-room spacing. Row/col
// are 0-indexed.
const EQUIP_GRID_LAYOUT = {
  offhand:{row:0,col:0}, head:{row:0,col:1},                     face:{row:0,col:3}, neck:{row:0,col:4},
  weapon:{row:1,col:0},                                                                chest:{row:1,col:4},
  foot:{row:3,col:0},                                                                  back:{row:3,col:4},
  legs:{row:4,col:0}, waist:{row:4,col:1},                       palm:{row:4,col:3}, wrist:{row:4,col:4},
};
...
// Renders the compass grid + full slot legend as plain HTML (no React — matches how the rest
// of the Letter of Records is built). Display-only: no data-id, no interaction, nothing here
// is ever read back into the sheet.
function renderEquipCompassHtml(items){
  ...
  const cells = EQUIP_SLOT_RULES.map(rule=>{
    const pos = EQUIP_GRID_LAYOUT[rule.slot];
    const item = bySlot[rule.slot];
    return `<div class="equip-cell" style="grid-row:${pos.row+1};grid-column:${pos.col+1};">
      <div class="equip-slot-icon${item ? ' is-filled' : ''}">${rule.icon}</div>
      <div class="equip-slot-label${item ? ' is-filled' : ''}">${escapeHtml(rule.label)}</div>
    </div>`;
  }).join('');
  const hub = `<div class="equip-hub" style="grid-row:2 / span 3;grid-column:2 / span 3;"><div class="equip-hub-icon">🧍</div></div>`;
  ...
  return `<div class="equip-compass-wrap">
    <div class="equip-grid">${hub}${cells}</div>
