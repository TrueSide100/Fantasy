...
function renderPanelHtml(panel, opts){
  ...
  // (Equipped/Worn/Sheathed/Held), inserted directly before the Inventory section it
  // summarizes — so on the Finances+Inventory tab the order reads Finances, then this,
  // then the full Inventory list. Skipped on the dedicated drag-to-merge Inventory-only
  ...
  if(!opts.enableMerge && !opts.hideSectionTitle){
    ...
    if(invIdx !== -1){
      ...
      sections.splice(invIdx, 0, equipHtml);
    }
  }
