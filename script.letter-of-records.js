    ...
    if(cat.type==='kv'){
      ...
      inner = mergeHintHtml + ((cat.data && cat.data.length)
        ? `<div class="${chipsClass}">${cat.data.map((it, i)=>{
            ...
            // of flowing text pills. A permanent #N corner tag, a stack-count badge, and —
            // when the item carries a status suffix — a vertical status tag all sit on top
            // of the image. Long-pressing a card (main Letter of Records view only — see
            // data-photo-editable below) opens the photo picker for that item; the dedicated
            // drag-to-merge Inventory page shows the same card read-only so it never fights
            // with its own long-press-to-drag gesture. Every other list category (Skills,
            // Scheduled Events, etc.) is untouched and keeps the original plain-text chip look.
            const photoUrl = isInventoryCat && panel.photos ? panel.photos[itemId] : null;
            const photoEditable = isInventoryCat && !mergeActive;
            const statusColHtml = (isInventoryCat && status)
              ? `<div class="inv-slot-status-col">
                   <span class="inv-slot-status-stack">${status.split('').map(ch => ch===' ' ? '&nbsp;' : escapeHtml(ch)).join('<br>')}</span>
                   ${hasQty ? `<span class="inv-slot-status-qty">×${parsedLabel.qty.toLocaleString('en-US')}</span>` : ''}
                 </div>`
              : '';
            const topRightQtyHtml = (isInventoryCat && !status && hasQty) ? qtyHtml : '';
            const chipHtml = isInventoryCat
              ? `<span class="panel-chip inv-slot${isPast ? ' is-past' : ''}"${idAttr} data-item-id="${escapeHtml(itemId)}"${photoEditable ? ' data-photo-editable="1"' : ''} data-slot-name="${escapeHtml(nameOnly)}${hasQty ? ' ×' + parsedLabel.qty.toLocaleString('en-US') : ''}">
...
// ---------- inventory item photos (long-press to add) ----------
// Long-pressing an inventory card opens a photo picker for that item and stores the result
// as a data URL on the panel itself (see setInventoryItemPhoto), the same way every other
// piece of item identity is persisted. Deliberately scoped to cards carrying
// data-photo-editable — set only on the main Letter of Records view (see renderPanelHtml) —
// so this never competes with the dedicated Inventory page's own long-press-to-drag gesture.
(function(){
  ...
  document.addEventListener('pointerdown', (e)=>{
    ...
    if(!card || e.target.closest('.panel-chip-del')) return;
    clearPress();
