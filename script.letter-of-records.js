...
function defaultPanel(){
  return { numSeq:0, numMap:{}, categories: {
...
function migrateOldPanel(old){
  ...
  const panel = { numSeq:0, numMap:{}, categories: {
...
function sanitizePanel(p){
  ...
  const out = { categories:{}, numSeq: typeof p.numSeq==='number' ? p.numSeq : 0, numMap: (p.numMap && typeof p.numMap==='object') ? p.numMap : {} };
...
async function getPanel(id){
  ...
  if(!p || migrated || ensured || dedupedLearning || promoted || repaired || idsBackfilled || legacyIdsMigrated) await savePanel(id, panel);
...
const EQUIP_ON_BODY_STATUSES = ['equipped', 'worn', 'sheathed', 'held'];
...
const EQUIP_SLOT_RULES = [
  // --- auto-guessed from item-name keywords, same mechanism as before ---
  { slot:'full-body', icon:'⬜', label:'Full body', kws:['bodysuit','jumpsuit','onesie','full-body','full body'] },
  { slot:'head', icon:'⬜', label:'Head / forehead', kws:['helm','kabuto','hat','hood','headband','crown','circlet','forehead'] },
  { slot:'face', icon:'⬜', label:'Face / eyes', kws:['mask','veil','goggles','blindfold','visor','glasses','monocle'] },
  { slot:'neck', icon:'⬜', label:'Neck', kws:['amulet','necklace','pendant','torc','choker'] },
  { slot:'chest', icon:'⬜', label:'Chest', kws:['armor','cuirass','robe','coat','jacket','vest','kimono','plate','breastplate'] },
  { slot:'back', icon:'⬜', label:'Back', kws:['cloak','cape','mantle','backpack','quiver','satchel'] },
  { slot:'waist', icon:'⬜', label:'Waist', kws:['belt','sash','girdle','waistband'] },
  { slot:'legs', icon:'⬜', label:'Legs', kws:['greaves','trousers','pants','leggings','hakama'] },
  { slot:'foot', icon:'⬜', label:'Foot / feet / ankle', kws:['boot','tabi','shoe','sandal','geta','anklet'] },

  // --- positional/manual: left vs right, upper vs lower, front vs back, wearing vs action
  // can't be told apart from an item's name alone, so these never auto-fill yet. Placeholder
  // icon only; will stay "Empty" until we decide how an item gets pinned to one of these. ---
  { slot:'wrist-upper-left', icon:'⬜', label:'Upper left wrist', kws:[] },
  { slot:'wrist-lower-left', icon:'⬜', label:'Lower left wrist', kws:[] },
  { slot:'wrist-front-right', icon:'⬜', label:'Front right wrist', kws:[] },
  { slot:'wrist-back-right', icon:'⬜', label:'Back right wrist', kws:[] },
  { slot:'upper-arm-left', icon:'⬜', label:'Left upper arm', kws:[] },
  { slot:'upper-arm-right', icon:'⬜', label:'Right upper arm', kws:[] },
  { slot:'forearm-left', icon:'⬜', label:'Left forearm', kws:[] },
  { slot:'forearm-right', icon:'⬜', label:'Right forearm', kws:[] },
  { slot:'hand-left-wearing', icon:'⬜', label:'Left hand wearing', kws:[] },
  { slot:'hand-left-action', icon:'⬜', label:'Left hand action', kws:[] },
  { slot:'hand-right-wearing', icon:'⬜', label:'Right hand wearing', kws:[] },
  { slot:'hand-right-action', icon:'⬜', label:'Right hand action', kws:[] },
  { slot:'thigh-left', icon:'⬜', label:'Left thigh', kws:[] },
  { slot:'thigh-right', icon:'⬜', label:'Right thigh', kws:[] },
];
...
// identically to every other Inventory-facing view.
function computeEquipSlots(items){
  const bySlot = {};
  const accessories = [];
  for(const raw of (items||[])){
    const it = splitItemEntry(raw);
    if(!isOnBodyStatus(it.status)) continue;
    const rule = guessEquipSlot(it.name);
    if(rule.slot === 'accessory'){ accessories.push(Object.assign({}, it, rule)); continue; }
    if(!bySlot[rule.slot]) bySlot[rule.slot] = Object.assign({}, it, rule);
  }
  return { bySlot, accessories };
}
// Renders the hub (person icon, left) + a simple 2-column grid of every slot (right), plus
// the full slot legend below, as plain HTML (no React — matches how the rest of the Letter
// of Records is built). Display-only: no data-id, no interaction, nothing here is ever read
// back into the sheet.
function renderEquipCompassHtml(items){
  const { bySlot, accessories } = computeEquipSlots(items);
  const cells = EQUIP_SLOT_RULES.map(rule=>{
    const item = bySlot[rule.slot];
    return `<div class="equip-cell">
      <div class="equip-slot-icon">${rule.icon}</div>
      <div class="equip-slot-label">${escapeHtml(rule.label)}</div>
    </div>`;
  }).join('');
  const hub = `<div class="equip-hub-top" onclick="document.getElementById('equipHubTopFile').click()"><div class="equip-hub-icon" id="equipHubTopIcon">🧍</div></div><div class="equip-hub-bottom" onclick="document.getElementById('equipHubBottomFile').click()"><span class="equip-hub-placeholder" id="equipHubBottomPlaceholder">+ Add image</span></div>`;
  const legendRows = EQUIP_SLOT_RULES.map(rule=>{
    const item = bySlot[rule.slot];
    return `<div class="equip-legend-row">
      <span class="equip-legend-icon${item ? ' is-filled' : ''}">${rule.icon}</span>
      <span class="equip-legend-label${item ? ' is-filled' : ''}">${escapeHtml(rule.label)}</span>
      <span class="equip-legend-name${item ? '' : ' is-empty'}">${item ? escapeHtml(item.name) : 'Empty'}</span>
    </div>`;
  }).join('');
  const accessoriesHtml = accessories.length
    ? `<div class="equip-accessories-title">Also carried</div>
       <div class="equip-accessories">${accessories.map(a=>`<span class="equip-accessory-chip">${escapeHtml(a.name)}</span>`).join('')}</div>`
    : '';
  return `<div class="equip-compass-wrap">
    ...
    <div class="equip-legend">${legendRows}</div>
    ${accessoriesHtml}
  </div>`;
}
...
// Relationships, Milestones, and anything not matched above, so a brand-new category the AI
// introduces later always lands somewhere instead of vanishing.
...
// ---------- stray duplicate inventory category suppression (already-saved data) ----------
// mergePanelUpdate above stops a new stray inventory-ish category from ever forming, but that
// doesn't retroactively fix a world that already has one saved (e.g. "Inventory Description"
// sitting alongside the real "Inventory"). This hides any such leftover — both from the
// rendered Letter of Records and from what's sent to the AI as context — without touching the
// real "Inventory" category or deleting the stray data outright. Never hides anything if no
// canonical "Inventory" category exists yet, since there'd be nothing to treat as "the real
// one" and hiding the only copy of the data would just lose it.
function isStrayInventoryCategory(name, categories){
  const trimmed = String(name || '').trim();
  if(!/^inventory/i.test(trimmed)) return false;
  const canonicalKey = findExistingKey(categories, 'Inventory');
  if(!canonicalKey) return false;
  return normalizeKey(canonicalKey) !== normalizeKey(trimmed);
}
function orderedCategoryEntries(panel){
  return Object.entries(panel.categories || {})
    .filter(([name]) => !isStrayInventoryCategory(name, panel.categories || {}))
...
function renderPanelHtml(panel, opts){
  ...
  const sections = filteredEntries.map(([name, cat])=>{
    ...
    if(cat.type==='kv'){
      ...
      const collapsible = (isInventoryCat && !mergeActive) || isSkillsCat || isSchedCat;
      ...
      inner = mergeHintHtml + ((cat.data && cat.data.length)
        ? `<div class="${chipsClass}">${cat.data.map((it, i)=>{
            ...
            const delBtn = mergeActive
              ...
              : '';
            // Inventory items render as square, game-style slot frames (icon glyph + a
            // permanent #N corner tag + a stack-count badge) instead of flowing text pills.
            // The full name is intentionally not printed on the tile itself — it surfaces as
            // an overlay label when the slot is tapped (see the delegated 'inv-slot' click
            // handler near the bottom of this file), same as an RPG inventory grid. Every
            ...
            const chipHtml = isInventoryCat
              ? `<span class="panel-chip inv-slot${isPast ? ' is-past' : ''}"${idAttr} data-slot-name="${escapeHtml(nameOnly)}${hasQty ? ' ×' + parsedLabel.qty.toLocaleString('en-US') : ''}">
                   ${numHtml}
                   <span class="inv-slot-icon">${escapeHtml(invSlotGlyph(nameOnly))}</span>
                   ${qtyHtml}
                   <span class="inv-slot-label">${escapeHtml(nameOnly)}${qtyHtml}</span>
                   ${delBtn}
                 </span>`
              : (mergeActive
  ...
  // Equip compass: a read-only visual summary of Inventory's own on-body statuses
  // (Equipped/Worn/Sheathed/Held), inserted directly after the Inventory section it
  // summarizes — so on the Finances+Inventory tab the order reads Finances, then the
  // full Inventory list, then this. Skipped on the dedicated drag-to-merge Inventory-only
  ...
  if(!opts.enableMerge && !opts.hideSectionTitle){
    ...
    if(invIdx !== -1){
      const equipHtml = renderPanelSection('Equip', renderEquipCompassHtml(filteredEntries[invIdx][1].data));
...
// Inventory grid slots show their icon glyph + badges by default and reveal the full
// item name only when tapped (see .inv-slot-label / .is-open in the CSS) — same as
// hovering an item in a game inventory grid. Shared between the main Letter of Records
// (#panelContent) and the dedicated drag-to-merge Inventory page (#invContent); on the
// latter, a tap on the delete cross or a chip mid-drag never triggers the reveal.
function handleInvSlotTap(e){
  ...
  if(slot.classList.contains('is-dragging') || slot.classList.contains('is-pressing')) return false;
  const wasOpen = slot.classList.contains('is-open');
  document.querySelectorAll('.panel-chip.inv-slot.is-open').forEach(s=>{ if(s!==slot) s.classList.remove('is-open'); });
  slot.classList.toggle('is-open', !wasOpen);
  return true;
}
document.addEventListener('click', (e)=>{
  // Any tap outside a slot closes whichever one is currently showing its name label.
  if(!e.target.closest('.panel-chip.inv-slot')){
    document.querySelectorAll('.panel-chip.inv-slot.is-open').forEach(s=> s.classList.remove('is-open'));
    return;
  }
