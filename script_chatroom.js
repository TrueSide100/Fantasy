...
// once it's held steady for a couple of checks in a row (or a hard timeout, so this can never
// run indefinitely on a chat that's still actively receiving something). Cause-agnostic on
// purpose — see the FIX(opens-at-first-message-2) comment above openStory's call to this.
function watchAndPinToBottom(maxMs=2000, intervalMs=120){
  ...
  const tick = () => {
    ...
    if(steady >= 2 || Date.now() - start > maxMs) return; // settled, or gave up watching
...
async function sendUserTurn(opts){
  ...
  // FIX(feels-stuck-on-send): checkClaimAgainstRecords' Layer 2 is a real AI round-trip
  // (up to 2 models, each retried once — see readClaimsFromInput/askAIWithRetry), and it
  // used to be awaited BEFORE the player's message ever appeared or the input box cleared.
  // On anything slower than an instant reply that read as the input being "stuck" — nothing
  // visibly happens for several seconds while the button just sits disabled. The message now
  // renders immediately (optimistic UI, same as any normal chat app); the claim check still
  // runs and can still block the send, it just does so AFTER the bubble is already visible —
  // if it flags an issue, that bubble is retracted and the text restored to the input, same
  // end result as before, just with instant feedback in the meantime.
  ...
  if(val && !isSystemTrigger(val)){
    ...
    if(claimIssue){
      // Retract the optimistic bubble and hand the text back, same net behavior as the old
      // pre-render block — the player can just edit and resend, nothing was ever saved. (A
      // media attachment, if any, is dropped along with it — the player can re-attach after
      // fixing the caption.)
      chat.pop();
      els.log.lastElementChild?.remove();
      els.textInput.value = val;
