...
async function openStory(id){
  ...
  // FIX(opens-at-first-message): that single pin above measures #log's height the instant
  // messages are in the DOM — but on a long story, real content height keeps changing for a
  // moment after that: (1) each bubble uses custom web fonts (Fraunces/Caveat/EB Garamond)
  // that render with fallback-font metrics first and then swap in, re-wrapping every line of
  // every message at once; (2) any image/video attachment in the history hasn't finished
  // loading yet and still occupies zero real height. Both grow #log's true scrollHeight AFTER
  // the scrollTop above was already set, and on a chat with many messages that leftover gap
  // can be large enough to land the visible viewport all the way back up near message #1
  // instead of the actual bottom. Re-pin once each of those settles, plus one cheap
  // double-rAF pass to catch whatever's still mid-layout a couple of frames later on slower
  // devices — all no-ops once the content's real height has already stopped changing.
  if(document.fonts && document.fonts.ready){
    document.fonts.ready.then(()=> pinToBottomAfterRender());
  }
  els.log.querySelectorAll('.msg-media').forEach(media=>{
    const evt = media.tagName === 'VIDEO' ? 'loadedmetadata' : 'load';
    media.addEventListener(evt, ()=> pinToBottomAfterRender(), {once:true});
  });
  requestAnimationFrame(()=> requestAnimationFrame(()=> pinToBottomAfterRender()));
}
