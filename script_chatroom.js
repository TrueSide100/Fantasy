...
async function detectUnrequestedTimeSkip(reply, playerText){
  if(!reply) return false;
  try{
    const bgModel = await getGeminiBgModel();
    const prompt = `Player's last message: """${playerText || '(nothing typed — story continuing on its own)'}"""\n\nStory reply just written: """${reply}"""\n\nDoes the story reply narrate time moving forward to a later point than where the scene stood — a night passing, morning arriving, hours/days elapsing, a scene cutting ahead — whether stated outright ("the next morning", "hours later") or only implied (the room going dark then light returning, the character drifting off and then waking up, a knock arriving after an unstated gap, a candle guttering out then sunlight appearing)? Only answer YES if the reply itself shows or implies real time has passed since the player's message AND the player's own message above did not explicitly ask for that. Answer with exactly one word: YES or NO.`;
    const answer = await askAIWithRetry('You are a precise story-continuity checker. Respond with exactly one word: YES or NO. Nothing else.', prompt, bgModel);
    return /^\s*yes\b/i.test(answer || '');
  }catch(e){
    console.warn('[time-skip semantic check] failed, skipping semantic check for this turn', e);
    return false; // fail open — don't block a reply just because the check itself errored
  }
}
// ---------- semantic backstop for NO SELF-INITIATED SYSTEM/GAME-INTERFACE EVENTS ----------
// Same story as detectUnrequestedTimeSkip below/above: the worldSystemPrompt rule is a soft
// instruction, and a narrator with a "System"-style premise baked into its lore can still slip
// and have it hand out a reward on its own, no player action required. There's no fixed phrase
// to phrase-scan for here (unlike time-skips, a "System" grant is entirely premise-specific —
// could be a reward, a notification, a level-up, anything), so this goes straight to a semantic
// check rather than trying a regex first.
async function detectUnrequestedSystemGrant(reply, playerText){
  if(!reply) return false;
  try{
    const bgModel = await getGeminiBgModel();
    const prompt = `Player's last message: """${playerText || '(nothing typed — story continuing on its own)'}"""\n\nStory reply just written: """${reply}"""\n\nThis world's premise includes a "System," status window, or similar game-like interface that can hand the player rewards, items, or notifications. Does the story reply above have that System/interface actively hand the player something, show a notification, or otherwise act — a daily reward, a granted item, a status popup, a level-up — WITHOUT the player's own message above explicitly asking for it, checking it, or opening/interacting with it this turn? Answer with exactly one word: YES or NO.`;
    const answer = await askAIWithRetry('You are a precise story-continuity checker. Respond with exactly one word: YES or NO. Nothing else.', prompt, bgModel);
    return /^\s*yes\b/i.test(answer || '');
  }catch(e){
    console.warn('[system-grant semantic check] failed, skipping check for this turn', e);
    return false; // fail open — don't block a reply just because the check itself errored
  }
}
// ---------- semantic backstop for ITEM GRANTS — SAY IT PLAINLY ----------
// Same idea as detectUnrequestedSystemGrant just below/above: worldSystemPrompt's itemGrantRule
// is a soft instruction, and the narrator can still slip into a purely cinematic beat — a box
// materializing, an object being "found" — without ever actually saying, in plain unambiguous
// language, that the player obtains/receives/gains it by name. That plain sentence is exactly
// what the record-keeping pass (updatePanel/PANEL_SYS_PROMPT) needs to reliably file the item
// into Inventory, so a reply that skips it isn't just a style miss — it's the actual reason a
// granted item can end up never reaching the sheet at all. No fixed phrase to scan for here
// (the missing thing is an absence, not a presence), so this goes straight to a semantic check.
async function detectMissingItemGrantPhrase(reply, playerText){
  if(!reply) return false;
  try{
    const bgModel = await getGeminiBgModel();
    const prompt = `Player's last message: """${playerText || '(nothing typed — story continuing on its own)'}"""\n\nStory reply just written: """${reply}"""\n\nDoes this reply narrate the player being given, finding, or receiving a new item — a gift, a find, a reward, loot, a box/chest/hand held out to them, a scroll/object materializing, anything physical changing hands or appearing for them to keep — WITHOUT also plainly stating, in unambiguous language naming the item, that the player obtains/receives/gains/is granted it (a real obtain-word, e.g. "you obtain X", "you receive X", "you've been granted X")? Only answer YES if an item-granting moment is clearly happening in the reply AND that plain naming sentence is genuinely missing — a reply that already says "you obtain/receive/gain/are granted <item>" should get a NO, even if the wording leading up to it is cinematic. Answer with exactly one word: YES or NO.`;
    const answer = await askAIWithRetry('You are a precise story-continuity checker. Respond with exactly one word: YES or NO. Nothing else.', prompt, bgModel);
    return /^\s*yes\b/i.test(answer || '');
  }catch(e){
    console.warn('[item-grant phrasing check] failed, skipping check for this turn', e);
    return false; // fail open — don't block a reply just because the check itself errored
  }
}
async function continueStory(world, chat, mode, panelOpts){
  ...
  try{
    ...
    // ---------- deterministic backstop for NO SELF-INITIATED TIME SKIPS ----------
    // The system-prompt rule alone is a soft instruction — the narrator can still slip and
    // narrate "the next morning"/"you wake up"/etc. on its own. This re-checks the actual reply
    // text with the same phrase-scan the Timeline guard already trusts (skipDaysInLine, from
    // script_tase.js — safe to call here, all files share one global scope). Only fires in
    // normal reply mode: 'forward' (the idle-continue path) is explicitly instructed to "advance
    // time" on its own, so this check would just fight that mode's own purpose and retry forever.
    if(mode !== 'forward'){
      const lastPlayerMsg = [...chat].reverse().find(m=>m.role==='user');
      const lastPlayerText = lastPlayerMsg ? (lastPlayerMsg.text || '') : '';
      const playerAskedForSkip = skipDaysInLine(lastPlayerText) > 0;
      if(!playerAskedForSkip){
        // Cheap phrase-scan first; only spend the extra AI call checking for an IMPLIED skip
        // (no matched phrase) if the phrase-scan itself came up empty.
        let flagged = skipDaysInLine(reply) > 0;
        if(!flagged) flagged = await detectUnrequestedTimeSkip(reply, lastPlayerText);
        if(flagged){
          const correctionNote = `\n\nYour previous attempt broke the NO SELF-INITIATED TIME SKIPS rule — it narrated (or implied) time passing, a night passing, or a new day/morning starting even though the player's own message didn't ask for that. Here is the reply you must NOT repeat:\n"""\n${reply}\n"""\nWrite a new reply instead that stays inside the exact same moment as "Story so far" above — continue right where the scene left off in response to what the player just did or said. No time-skip, no falling asleep and waking up, no "next morning," no new day, no jump forward of any kind, stated or implied.`;
          try{
            const fixedReply = await askAI(worldSystemPrompt(world, memory, panel), `Story so far:\n${recentLog}\n\n${instruction}${finalReminder}${correctionNote}`);
            if(fixedReply) reply = fixedReply; // one retry only — accept it as-is rather than looping further
          }catch(e){ console.warn('[time-skip retry] failed, keeping original reply', e); }
        }
      }
    }
    // ---------- deterministic backstop for NO SELF-INITIATED SYSTEM/GAME-INTERFACE EVENTS ----------
    // Unlike the time-skip check above, this one runs in EVERY mode, including 'forward' — there's
    // no legitimate case where the idle-continue path should have the System hand out a reward on
    // its own either, so there's no mode to carve out an exception for.
    {
      const lastPlayerMsgSys = [...chat].reverse().find(m=>m.role==='user');
      const lastPlayerTextSys = lastPlayerMsgSys ? (lastPlayerMsgSys.text || '') : '';
      const unrequestedSystemGrant = await detectUnrequestedSystemGrant(reply, lastPlayerTextSys);
      if(unrequestedSystemGrant){
        const correctionNoteSys = `\n\nYour previous attempt broke the NO SELF-INITIATED SYSTEM/GAME-INTERFACE EVENTS rule — it had the System/interface hand out a reward, item, or notification even though the player's own message didn't ask for, check, or open it this turn. Here is the reply you must NOT repeat:\n"""\n${reply}\n"""\nWrite a new reply instead that stays inside the exact same moment as "Story so far" above and responds to what the player actually did or said — with NO System/interface activity of any kind (no reward, no popup, no notification, no item grant from it) unless the player's own message explicitly invoked it this turn.`;
        try{
          const fixedReplySys = await askAI(worldSystemPrompt(world, memory, panel), `Story so far:\n${recentLog}\n\n${instruction}${finalReminder}${correctionNoteSys}`);
          if(fixedReplySys) reply = fixedReplySys; // one retry only — accept it as-is rather than looping further
        }catch(e){ console.warn('[system-grant retry] failed, keeping original reply', e); }
      }
    }
    // ---------- deterministic backstop for ITEM GRANTS — SAY IT PLAINLY ----------
    // Runs whenever this world tracks a sheet (panel) at all — itemGrantRule above is only
    // added to the system prompt when panel exists, so this check is scoped the same way.
    // Fires in every mode, same reasoning as the system-grant check just above: an idle
    // "Forward" continuation can just as easily have an NPC hand the player something as a
    // reply to direct action can.
    if(panel){
      const lastPlayerMsgGrant = [...chat].reverse().find(m=>m.role==='user');
      const lastPlayerTextGrant = lastPlayerMsgGrant ? (lastPlayerMsgGrant.text || '') : '';
      const missingGrantPhrase = await detectMissingItemGrantPhrase(reply, lastPlayerTextGrant);
      if(missingGrantPhrase){
        const correctionNoteGrant = `\n\nYour previous attempt broke the ITEM GRANTS — SAY IT PLAINLY rule — it narrated the player being given, finding, or receiving an item without also plainly stating, using a real obtain-word, that the player obtains/receives/gains/is granted it by name. Here is the reply you must NOT repeat as-is:\n"""\n${reply}\n"""\nWrite a new reply that keeps the same moment and events, but ALSO makes sure somewhere in it you plainly state the grant in unambiguous language naming the item — e.g. "You obtain a Scroll of Water Walking." / "You receive 3 explosive tags." It's fine to keep the cinematic lead-in (a box materializing, something dropping into your lap, etc.) — just add the plain obtain sentence right after it, naming the item exactly.`;
        try{
          const fixedReplyGrant = await askAI(worldSystemPrompt(world, memory, panel), `Story so far:\n${recentLog}\n\n${instruction}${finalReminder}${correctionNoteGrant}`);
          if(fixedReplyGrant) reply = fixedReplyGrant; // one retry only — accept it as-is rather than looping further
        }catch(e){ console.warn('[item-grant phrasing retry] failed, keeping original reply', e); }
      }
    }
  }catch(err){
    hideTyping();
...
async function updatePanel(world, opts){
  ...
  try{
    ...
    // ---------- backstop: background pass missed an obvious item-grant phrase ----------
    // itemGainConfirmed/guardStackableItems (script_inventory_equip.js) only ever VALIDATE an
    // Inventory list_add the background pass above already proposed — same limitation as the
    // Learning backstop just above, just for items instead of skills. When the pass simply
    // fails to notice a plain "you obtain/receive/are granted an X" line in the AI's own
    // reply — model inconsistency, not a rule — nothing gets proposed and there's nothing for
    // the Inventory guards to check, so the item silently never reaches the sheet even though
    // the reply (and worldSystemPrompt's ITEM GRANTS rule) already committed to giving it out
    // loud. This gives it one targeted second look specifically at that gap: if the latest AI
    // reply reads like a grant (systemGrantDetected, from script_inventory_equip.js — safe to
    // reference here, all guard files share one global scope) and the background pass's own
    // JSON didn't add anything to Inventory this turn, ask a narrowly-scoped follow-up to name
    // exactly what was granted, then merge just that in. The normal Inventory guards further
    // below (guardStackableItems etc.) still run on the result afterward exactly as if the
    // background pass had proposed it directly, so a hallucinated or already-owned item still
    // gets filtered out the same way.
    const lastAiMsg = [...chat].reverse().find(m=>m.role==='ai');
    const lastAiText = lastAiMsg ? (lastAiMsg.text || '') : '';
    if(lastAiText && typeof systemGrantDetected === 'function' && systemGrantDetected(lastAiText)){
      const proposedInventoryAdd = data && data.categories && Object.entries(data.categories)
        .some(([catName, catUpdate]) => /^inventory/i.test(catName.trim()) && catUpdate && Array.isArray(catUpdate.list_add) && catUpdate.list_add.length > 0);
      if(!proposedInventoryAdd){
        try{
          const grantFollowupPrompt = `${prompt}\n\nDouble-check one thing before finalizing: the story reply just written was:\n"""${lastAiText}"""\nThat reply reads like it grants, gives, or awards the player something (a "you obtain/receive/are granted/gain..." line, a System-style notification, a gift/reward/loot moment). If it actually names a specific new item (or more of an item already on the sheet) that isn't already reflected in "Inventory" on the current sheet above, make sure your JSON output includes that as an Inventory list_add — output the SAME JSON shape as before, this time including that Inventory update if it genuinely belongs. If the reply doesn't actually name a concrete item (just a vague mood/description with nothing to add), or the item is already on the sheet, it's correct to leave Inventory out entirely — only add it if it genuinely applies.`;
          const rawGrant = await askAIWithRetry(PANEL_SYS_PROMPT, grantFollowupPrompt, bgModel);
          const dataGrant = extractJsonObject(rawGrant);
          if(dataGrant && dataGrant.categories){
            for(const [catName, catUpdate] of Object.entries(dataGrant.categories)){
              if(!/^inventory/i.test(catName.trim()) || !catUpdate || !Array.isArray(catUpdate.list_add) || !catUpdate.list_add.length) continue;
              data = data || {categories:{}};
              data.categories = data.categories || {};
              data.categories[catName] = data.categories[catName] || {};
              data.categories[catName].list_add = [...(data.categories[catName].list_add || []), ...catUpdate.list_add];
            }
          }
        }catch(e){ console.warn('[item-grant retry] failed, proceeding without it', e); }
      }
    }
    ...
    guardCurrencyDecreases(data, panel, recentPlayerText, recent);
