...
// Bumped once per openStory() call. watchAndPinToBottom checks this (alongside
// state.chattingId) on every fire so a still-running watch from a PREVIOUS open can't
...
async function openStory(id){
  ...
  els.log.innerHTML = '';
  // Each world's draft input is independent — never let text typed in one world
  // leak into another when switching between them.
  els.textInput.value = '';
  ...
  // #log's real height keeps changing for a moment after the messages are in the DOM —
  // custom webfonts swapping in and re-wrapping every bubble, images/video still decoding —
  // and on a big chat (300+ messages) that can take a while on a slow device. Rather than
  // guess how long "a while" is, watchAndPinToBottom below watches #log itself for actual
  // size changes and re-pins every time one happens, for as long as it's watching.
  watchAndPinToBottom(id, myOpenGen);
}
// Re-pins to bottom every time #log's real size changes (font swap, image/video finishing
// load, anything else) — driven by ResizeObserver, not a timer, so there's no interval or
// timeout to under-guess on a large chat. Stops the moment the user scrolls/touches the
// chat themselves, or leaves this chat, or (pure safety net, not the normal stop condition)
// after a minute of no activity.
function watchAndPinToBottom(id, myOpenGen){
  if(!els.log || !els.chatBody) return;
  let stopped = false;
  const ro = new ResizeObserver(()=>{
    if(stopped) return;
    if(id !== undefined && (state.chattingId !== id || openStoryGeneration !== myOpenGen)){ stop(); return; }
    pinToBottomAfterRender();
  });
  const stop = ()=>{
    if(stopped) return;
    stopped = true;
    ro.disconnect();
    els.chatBody.removeEventListener('touchstart', stop);
    els.chatBody.removeEventListener('wheel', stop);
    clearTimeout(safetyTimer);
  };
  els.chatBody.addEventListener('touchstart', stop, {once:true, passive:true});
  els.chatBody.addEventListener('wheel', stop, {once:true, passive:true});
  const safetyTimer = setTimeout(stop, 60000);
  ro.observe(els.log);
}
...
// Which of the six bottom tabs is currently showing. Module-level so a background repaint
...
async function openPanelModal(targetTab){
  if(!state.chattingId) return;
  ...
  activePanelTab = targetTab && targetTab >= 1 && targetTab <= 6 ? targetTab : 1;
  els.panelTabBar.querySelectorAll('.panel-tab').forEach(b=> b.classList.toggle('active', parseInt(b.dataset.tab,10) === activePanelTab));
  await paintPanel();
...
// ================= "Finance & Gear" quick view (#econBtn / #econModal) =================
// A second, smaller icon next to the Letter of Records button. Opens a dedicated two-page
// sheet scoped to just the economy-related groups — page 1 is Finance+Holdings (tab 2 in
// panelTabForCategory), page 2 is Inventory+Equip (tab 3) — reusing the exact same
// renderPanelHtml({tabFilter}) rendering the main six-tab sheet already uses, just with its
// own two-button footer instead of the full six-tab bar.
let activeEconPage = 2;
async function paintEconModal(){
  if(!state.chattingId) return;
  const panel = await getPanel(state.chattingId);
  els.econContent.innerHTML = renderPanelHtml(panel, { tabFilter: activeEconPage });
}
async function openEconModal(){
  if(!state.chattingId) return;
  activeEconPage = 2;
  els.econTabBar.querySelectorAll('.panel-tab').forEach(b=> b.classList.toggle('active', parseInt(b.dataset.page,10) === activeEconPage));
  await paintEconModal();
  showOverlayModal(els.econModal);
  pushNavState('modal');
}
els.econBtn.onclick = openEconModal;
els.closeEconBtn.onclick = ()=>{ hideOverlayModal(els.econModal); };
els.econModal.onclick = (e)=>{ if(e.target===els.econModal){ hideOverlayModal(els.econModal); } };
els.econTabBar.addEventListener('click', (e)=>{
  const btn = e.target.closest('.panel-tab');
  if(!btn) return;
  const page = parseInt(btn.dataset.page, 10);
  if(page === activeEconPage) return;
  activeEconPage = page;
  els.econTabBar.querySelectorAll('.panel-tab').forEach(b=> b.classList.toggle('active', b === btn));
  const scrollEl = els.econContent.parentElement; // .sheet-scroll — snap back to top on page switch
  if(scrollEl) scrollEl.scrollTop = 0;
  paintEconModal();
});
...
els.textInput.addEventListener('input', resizeTextInput);
resizeTextInput();
// The page loads Inter via Google Fonts with display=swap, so this first call above can
// run against the fallback font's line metrics before Inter finishes downloading. Once it
// swaps in, the fixed pixel height we already set is stale (based on the old metrics) and
// clips the line until the user types and resizeTextInput runs again. Re-run it once the
// real font is actually ready, plus on window load as a fallback for browsers without the
// Font Loading API.
if(document.fonts && document.fonts.ready){
  document.fonts.ready.then(resizeTextInput);
}
window.addEventListener('load', resizeTextInput);
