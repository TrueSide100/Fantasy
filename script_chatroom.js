...
async function detectStoryRuleViolations(reply, playerText){
  ...
  try{
    ...
    console.warn('[story-rule semantic check] failed, skipping semantic check for this turn', e);
    return {timeSkip:false, systemGrant:false}; // fail open — don't block a reply just because the check itself errored
  }
}
...
async function updatePanel(world, opts){
  ...
  try{
    const bgModel = await getGeminiBgModel();
    // Extra outer-level retry loop on top of whatever askAIWithRetry already does internally —
    // "WithRetry" notwithstanding, it can still come back with genuinely nothing on a very poor
    // connection (0.00-1 KB/s in the field is basically no connection at all). Three attempts
    // with a real pause between them (2.5s, then 5s) gives a connection that's merely
    // intermittent — coming and going, not permanently dead — an actual chance to catch itself
    // before this gives up and shows the player a warning.
    let raw, lastCallErr;
    for(let attempt=0; attempt<3; attempt++){
      try{ raw = await askAIWithRetry(PANEL_SYS_PROMPT, prompt, bgModel); lastCallErr = null; break; }
      catch(e){ lastCallErr = e; if(attempt<2) await new Promise(r=>setTimeout(r, 2500*(attempt+1))); }
    }
    if(lastCallErr) throw lastCallErr;
  ...
  }catch(e){ showBackgroundWarning(world.id, 'Letter of records', e, ()=>updatePanel(world, opts)); }
}
...
// wrong chat.
function showBackgroundWarning(worldId, label, err, retryFn){
  ...
  if(retryFn){
    // A one-tap retry beats either silently giving up (the original problem: an item narrated
    // as obtained never making it onto the sheet, with no visible sign anything went wrong) or
    // blindly retrying forever in the background (which just delays the same failure on a truly
    // dead connection). Letting the player retry once the connection looks better to them is a
    // better fit than the app guessing when that moment is.
    const retryBtn = document.createElement('button');
    retryBtn.type = 'button'; retryBtn.className = 'warn-retry-btn'; retryBtn.textContent = 'Retry';
    retryBtn.style.marginLeft = '8px';
    retryBtn.onclick = async () => {
      retryBtn.disabled = true; retryBtn.textContent = 'Retrying…';
      try{ await retryFn(); w.remove(); } // success — clear the warning entirely
      catch(e){ retryBtn.disabled = false; retryBtn.textContent = 'Retry'; console.error(`[${label} retry failed]`, e); }
    };
