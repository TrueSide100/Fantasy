...
async function openPanelModal(targetTab){
  if(!state.chattingId) return;
  // A quick-access icon opens straight to its own group; otherwise (no tab specified)
  // default back to tab 1 (Identity) — same as before.
...
els.quickTabsBar.addEventListener('click', (e)=>{
  const btn = e.target.closest('.quick-tab-btn');
  if(!btn) return;
  openPanelModal(parseInt(btn.dataset.tab, 10));
});
