...
els.textInput.addEventListener('input', ()=>{
  ...
  _taResizeRAF = requestAnimationFrame(()=>{
    ...
    const maxH = Math.round(window.innerHeight * 0.35);
    // A trailing "\n" (just pressed Enter, cursor on a new empty line) isn't
    // reflected in scrollHeight until another character is typed on that line,
    // so the box doesn't grow yet and the prior line looks clipped at the
    // bottom edge. Pad by one line-height whenever the value ends in a newline
    // so the box grows the instant Enter is pressed, not one keystroke later.
    const cs = getComputedStyle(els.textInput);
    let lineH = parseFloat(cs.lineHeight);
    if(isNaN(lineH)) lineH = parseFloat(cs.fontSize) * 1.4;
    const endsWithNewline = /\n$/.test(els.textInput.value);
    const target = els.textInput.scrollHeight + (endsWithNewline ? lineH : 0);
    els.textInput.style.height = Math.min(target,maxH)+'px';
  });
});
