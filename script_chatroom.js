...
async function continueStory(world, chat, mode, panelOpts){
  ...
  try{
    ...
    if(mode !== 'forward'){
      ...
      const playerAskedForSkip = lastPlayerMsg ? skipDaysInLine(lastPlayerMsg.text || '') > 0 : false;
      if(!playerAskedForSkip && skipDaysInLine(reply) > 0){
        const correctionNote = `\n\nYour previous attempt broke the NO SELF-INITIATED TIME SKIPS rule — it narrated time passing or a new day/morning starting even though the player's own message didn't ask for that. Here is the reply you must NOT repeat:\n"""\n${reply}\n"""\nWrite a new reply instead that stays inside the exact same moment as "Story so far" above — continue right where the scene left off in response to what the player just did or said. No time-skip, no "next morning," no new day, no jump forward of any kind.`;
        try{
          const fixedReply = await askAI(worldSystemPrompt(world, memory, panel), `Story so far:\n${recentLog}\n\n${instruction}${finalReminder}${correctionNote}`);
          if(fixedReply && skipDaysInLine(fixedReply) === 0) reply = fixedReply;
