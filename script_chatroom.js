...
async function updatePanel(world, opts){
  ...
  try{
    ...
    // ensureTimelineDayFallback deliberately gets ONLY this turn's own two messages, not the
    // full 10-message `recent` window used everywhere else above. It uses its skip-phrase scan
    // as the actual amount to ADD (not just a ceiling, unlike guardTimelineDay's use of the same
    // scan below) — scanning the wide window would re-count every still-present older turn's
    // skip phrasing on every subsequent call and snowball Current Day upward turn after turn,
    // even with nothing new having happened. Scoping to just [lastPlayerMsg, latest reply]
    // means each turn only ever contributes its own, single, un-repeated skip amount.
    const thisTurnText = chat.slice(-2).map(messageToLogLine).join('\n');
    ensureTimelineDayFallback(data, panel, thisTurnText, world.id);
