/* ================================================================================
   ...
   The long-press-to-attach-media gesture on els.sendBtn and the sendMessage()
   click handler it shares that button with both live together near the bottom
   of Part 2.2 (the composer). Load order between them doesn't matter: the
   synthetic 'click' a long-press's release fires on that same button is
   suppressed via a one-shot flag the click handler checks and clears, not via
...
async function reconcilePanelFromMemory(world, memory, panel){
  ...
  try{
    ...
    // FIX(4)/script_finance.js: guardCurrencyIncreases now takes playerText as a 4th arg (used
    // for the sell/trade-for-money confirmation path). No player action backs this pass, so ''
    // is correct here — same "block the gain without an explicit confirmation" stance already
    // documented above for this call site.
    guardCurrencyIncreases(data, panel, memory, '');
    guardCurrencyRenameBypass(data, panel, memory);
    // Holdings (script_finance.js): still requires the SAME player-confirmed currency+quantity
    // affordability check as every other call site (holdingAffordabilityIssue) — and since
    // playerText is '' here (no player action backs this pass), that check can never pass, so a
    // Holdings gain never actually lands via this reconciliation pass. Still called for the
    // list_remove/kv/rename lockdown half, which always applies regardless of playerText.
    guardHoldingsAcquisition(data, panel, memory, '');
...
// Renders the player's own message exactly as typed: escape HTML for safety, then turn
// each newline into a <br> so multi-line input keeps its original line breaks. Unlike
// formatStoryText (below), this never re-splits on sentence punctuation, never regroups
// text into multi-sentence <p> paragraphs, and never treats quoted text as special
// "dialogue" — all of that is narration-formatting for AI replies and was, until now,
// also being applied to the player's own typed lines, which is why a bubble could come
// out looking different from what was actually typed (lines merged/rewrapped, quotes
// re-styled, etc). A single wrapping <p> keeps the same bubble spacing as before.
function formatUserText(s){
  if(!s) return '';
...
This applies even if the player's own message assumes or asserts they can do something the sheet doesn't support — the player narrating an action doesn't make it true. Push back through the story itself (an NPC's reaction, a failed attempt, a moment of realization) rather than breaking immersion to explain the rule directly.\n- DATA OVER MEMORY: the character sheet above is a live, exact record — always more current and more trustworthy than "What has happened in the story so far" or anything you might recall from earlier turns. If the memory summary below ever seems to imply a different number, item, or ability than the sheet actually lists, the sheet wins, no exceptions. Base this reply on the sheet as written, not on a general impression of how the story "should" have gone.` : '';
  ...
  const itemGrantRule = panel ? `\n\n=== ITEM GRANTS — SAY IT PLAINLY ===\nWhenever this reply gives the player a new item, or more of an item they already have — loot, a gift, a find, a reward, a box/chest/hand held out to them, anything — a vivid description of the moment is not enough on its own. Somewhere in that same reply you must ALSO state the grant in plain, unambiguous language naming the item, using a real obtain-word: "You obtain a set of ninja tools." / "You receive 3 explosive tags." / "You've been granted a Steel Kunai." It's fine to lead with the cinematic beat first ("A small box lands in your lap.") as long as the very next sentence plainly says the player obtains/receives/is granted/gains it by name. This is a strict requirement, not a style suggestion — the item will not actually reach the player's sheet unless this phrasing is there.` : '';
  const noSelfTimeSkipRule = `\n\n=== NO SELF-INITIATED TIME SKIPS ===\nYou never advance time or start a new day/morning on your own initiative. Do not write "the next morning," "the next day," "hours later," "when you wake up," "days pass," or any other line that skips time forward — UNLESS the player's own message you're replying to explicitly asked for or described that skip (e.g. "I sleep until morning," "let's skip ahead two days," "I wait a few days," "I go to bed for the night"). If the player's message did not ask for time to pass, keep this reply inside the same moment — continue the current scene forward in real time, never fast-forward the player falling asleep and waking up later, and never describe a day as having changed. The player is the only one who starts a time-skip; you only ever narrate the one they just asked for, nothing bigger and nothing on your own.`;
  ...
  const noSelfInitiatedSystemRule = `\n\n=== NO SELF-INITIATED SYSTEM/GAME-INTERFACE EVENTS ===\nIf this world's premise includes a "System," status window, game-like interface, daily reward mechanic, notification, level-up, or any other meta-game device that hands the player things or information, you never trigger it on your own initiative. Only narrate it doing something — granting a reward, showing a notification, leveling something up, opening a menu — when the player's own message THIS TURN explicitly asks for it, checks it, or interacts with it (e.g. "I check the System," "I open my status," "I see if I got today's reward"). If the player's message didn't do that, the System stays completely silent for this reply — don't have it interrupt the scene with an unprompted gift, popup, chime, or announcement, no matter how much time has passed or how fitting the moment feels.`;
  return `You are the narrator and every character of this story world.\n\nWorld & characters:\n${world.lore || '(no details given — invent something fitting the world name)'}\n\nRules: narrate in strict second person ("you") from the player's own point of view only. Describe only what the player can directly see, hear, or sense. Voice other characters only through visible dialogue and action, never their private thoughts. Always wrap every spoken line of dialogue in double quotation marks ("like this"), exactly as a character would say it aloud — never leave spoken words unquoted. No stage directions in asterisks; weave narration and dialogue naturally. Keep replies immersive but concise — 2-5 sentences unless a scene truly calls for more.${storyCommitmentRule}${memoryPermanenceRule}${panelCtx}${hardLimitRule}${categoryRulesText}${itemGrantRule}${noSelfTimeSkipRule}${noSelfInitiatedSystemRule}${memCtx}${scheduledEventTriggerRule}`;
}

// Bumped once per openStory() call. watchAndPinToBottom's poll checks this (alongside
// state.chattingId) on every tick so a still-running watch from a PREVIOUS open can't
// keep re-pinning after the user has since left that chat or opened another one.
let openStoryGeneration = 0;
async function openStory(id){
  const world = await getWorld(id);
  if(!world) return;
  state.chattingId = id;
  const myOpenGen = ++openStoryGeneration;
  ...
  // FIX(opens-at-first-message-2): that single pin above measures #log's height the instant
  // messages are in the DOM — but on a long story, real content height keeps changing for a
  // moment after that: custom web fonts (Fraunces/Caveat/EB Garamond) swapping in and
  // re-wrapping every bubble, or an image/video attachment that hasn't finished loading yet.
  // Both grow #log's true scrollHeight AFTER scrollTop was already set, and on a chat with
  // many messages that leftover gap can be large enough to land the viewport all the way
  // back up near message #1 instead of the bottom. The previous version chased this by
  // listening for specific known triggers (document.fonts.ready, a 'load'/'loadedmetadata'
  // per media element, a fixed double-rAF) — still not catching every case (a webfont whose
  // swap doesn't line up with fonts.ready resolving, a blob-URL image that decodes slower
  // than its 'load' event suggests, or anything else that grows the layout late). Rather than
  // enumerate more individual causes, watchAndPinToBottom below just polls #log's actual
  // scrollHeight directly and re-pins whenever it changes, regardless of why — it settles on
  // its own once the height stops moving, so this covers every case above at once plus
  // whatever else might grow it later.
  watchAndPinToBottom(id, myOpenGen);
}
// Polls #chatBody's real scrollHeight and re-pins to bottom every time it changes, stopping
// once it's held steady for a few checks in a row AFTER webfonts have actually finished
// loading (document.fonts.ready) — or once the user starts scrolling on their own, or a
// generous hard timeout, whichever comes first. Cause-agnostic on purpose — see the
// FIX(opens-at-first-message-2) comment above openStory's call to this.
//
// FIX(opens-at-first-message-4): a fixed few-second guess (tried in -2 and -3) still loses
// on a genuinely large chat (300+ messages, long bubbles) — the one-time webfont reflow
// across that much text can take longer than any flat guess on a slow device, especially
// the first time those fonts are loading from the network rather than cache. Rather than
// raise the guess again, gate "settled" on the real signal (fonts actually ready) and only
// use time as a generous outer safety cap, not the primary stop condition.
function watchAndPinToBottom(id, myOpenGen, maxMs=15000, intervalMs=150){
  let lastHeight = -1, steady = 0;
  // If the Font Loading API isn't available at all, don't gate on it — just fall back to
  // the old steady-reads-only behavior for that browser.
  ...
  // The instant the player scrolls or touches the log themselves, this stops re-pinning —
  // a long settle window must never fight a deliberate scroll, which is exactly the
  // flicker/snap-back bug the per-message pin removal (see comment above chat.forEach)
  // already fixed once; this is the same rule applied to the new, longer watch window.
  let userTookControl = false;
  const stopWatching = ()=>{ userTookControl = true; };
  els.chatBody.addEventListener('touchstart', stopWatching, {once:true, passive:true});
  els.chatBody.addEventListener('wheel', stopWatching, {once:true, passive:true});
  const cleanup = ()=>{
    els.chatBody.removeEventListener('touchstart', stopWatching);
    els.chatBody.removeEventListener('wheel', stopWatching);
  };
  const start = Date.now();
  const tick = () => {
    if(!els.chatBody){ cleanup(); return; }
    // Bail out quietly if the user has since left this chat or opened another one —
    // never fight navigation or a deliberate scroll on a chat we no longer own.
    if(id !== undefined && (state.chattingId !== id || openStoryGeneration !== myOpenGen)){ cleanup(); return; }
    if(userTookControl){ cleanup(); return; }
    const h = els.chatBody.scrollHeight;
    if(h !== lastHeight){ lastHeight = h; steady = 0; pinToBottomAfterRender(); }
    else steady++;
    const settled = steady >= 3 && fontsReady;
    if(settled || Date.now() - start > maxMs){ cleanup(); return; } // settled for real, or gave up watching
...
// Silently swaps a message's visible bubble text after the fact — used only by the background
// story-rule correction pass in continueStory. If the player has navigated away from this chat
// (or this exact bubble scrolled out and got re-rendered under a different element), the query
// just finds nothing and this is a no-op: the corrected text is already saved to `chat`/storage
// either way, so the next time this chat opens it renders correctly regardless.
function updateRenderedMsgText(index, newText){
  if(!els.log) return;
  const wrap = els.log.querySelector(`.msg[data-msgidx="${index}"]`);
  if(!wrap) return;
  const bubble = wrap.querySelector('.bubble');
  if(!bubble) return;
  const mediaEl = bubble.querySelector('.msg-media');
  bubble.innerHTML = (mediaEl ? mediaEl.outerHTML : '') + formatStoryText(newText);
}
function renderMsg(m, isLast, index, pin=true){
  const wrap = document.createElement('div');
  const kind = m.role==='user' ? 'user' : (m.role==='system' ? 'system' : 'ai');
  wrap.className = 'msg ' + kind;
  if(typeof index === 'number') wrap.dataset.msgidx = String(index);
  ...
  const mediaHtml = m.media
    ...
    : '';
  const textHtml = m.text ? (kind==='system' ? renderMarkdownLite(m.text) : (kind==='user' ? formatUserText(m.text) : formatStoryText(m.text))) : '';
...
// ---------- semantic backstop for NO SELF-INITIATED TIME SKIPS ----------
// skipDaysInLine (script_tase.js) only catches LITERAL phrases ("next morning", "overnight",
// "a week later"...). A narrator can just as easily imply the same skip with no trigger phrase
// at all — "the room grows dark and still... bright morning sunlight spills through" narrates a
// full night passing without ever saying "night" or "morning" in a matched form. A phrase-scan
// can never close that gap by adding more words to the regex; it needs an actual read of the
// text. This is a small, separate AI call (same background-model pattern as updateMemory/
// updatePanel) asked one strict yes/no question — cheap and fast next to the main story call.
// ---------- combined semantic backstop: TIME SKIPS + SYSTEM/GAME-INTERFACE EVENTS ----------
// This used to be two separate functions (detectUnrequestedTimeSkip, detectUnrequestedSystemGrant)
// each making its own background AI call. Merged into one call asking both questions at once —
// same detection coverage, half the network round-trips. That matters because every extra
// background call is one more thing that can fail outright on a flaky connection: this is what
// showed up as a "Letter of records update skipped — no reply came back" warning once this app
// started making more background calls per turn (the panel/Letter-of-records sync call was
// already there; these checks stacked more on top of it). Fewer calls per turn = fewer chances
// for a bad connection to drop one.
async function detectStoryRuleViolations(reply, playerText){
  if(!reply) return {timeSkip:false, systemGrant:false};
  const prompt = `Player's last message: """${playerText || '(nothing typed — story continuing on its own)'}"""\n\nStory reply just written: """${reply}"""\n\nAnswer two yes/no questions about the story reply above. Respond in EXACTLY this two-line format, nothing else:\nTIMESKIP: YES or NO\nSYSTEMGRANT: YES or NO\n\nTIMESKIP is YES if the reply narrates time moving forward to a later point than where the scene stood — a night passing, morning arriving, hours/days elapsing, a scene cutting ahead — whether stated outright ("the next morning", "hours later") or only implied (the room going dark then light returning, the character drifting off and then waking up, a candle guttering out then sunlight appearing) — AND the player's own message above did not explicitly ask for that. Otherwise NO.\n\nSYSTEMGRANT is YES only if this world's premise includes a "System," status window, or similar game-like interface, AND the reply has that System/interface actively hand the player something, show a notification, or otherwise act (a reward, a granted item, a popup, a level-up) WITHOUT the player's own message above explicitly asking for it, checking it, or opening/interacting with it this turn. If there's no such System premise in play at all, answer NO.`;
  // Two attempts before giving up. This check used to fail open on the FIRST error (a single
  // dropped/overloaded background call was enough to let a real, implied time-skip stand
  // uncorrected with zero visible sign anything was missed). One retry costs a single extra
  // background call only on the already-rare failure case, and closes that gap for the common
  // "transient blip" failure without turning a real, persistent outage into a blocked turn.
  for(let attempt=1; attempt<=2; attempt++){
    try{
      const bgModel = await getGeminiBgModel();
      const answer = await askAIWithRetry('You are a precise story-continuity checker. Respond in exactly the two-line format requested — nothing else.', prompt, bgModel);
      return {
        timeSkip: /TIMESKIP:\s*yes/i.test(answer || ''),
        systemGrant: /SYSTEMGRANT:\s*yes/i.test(answer || '')
      };
    }catch(e){
      if(attempt === 2){
        console.warn('[story-rule semantic check] failed twice, skipping semantic check for this turn', e);
        return {timeSkip:false, systemGrant:false}; // fail open only after a second failure
      }
      console.warn('[story-rule semantic check] failed once, retrying', e);
    }
  }
}
async function continueStory(world, chat, mode, panelOpts){
  ...
  renderMsg({role:'ai', text:reply}, true, msgIndex);
  await saveChat(world.id, chat);
  els.sendBtn.disabled = false; isSending = false;
  // ---------- background correction pass: NO SELF-INITIATED TIME SKIPS + SYSTEM/GAME-INTERFACE EVENTS ----------
  // This used to run INLINE above, before the reply was ever shown — meaning the typing
  // indicator and disabled send button sat through up to 3 sequential AI calls (story reply,
  // rule check, correction retry) instead of just 1. On a slow connection that's exactly what
  // showed up as a stuck input bar / frozen send button. Moved into the same background queue
  // updateMemory/updatePanel already use below: the reply now shows the instant the ONE main
  // call finishes, and if this check finds a violation a moment later, it silently rewrites the
  // already-shown message (in `chat`, and in the visible bubble too if this chat is still open)
  // instead of making the player wait for it up front. Runs BEFORE updateMemory/updatePanel in
  // this same queued task specifically so the Letter of Records sync reads whatever the
  // corrected text ends up being, not the original flawed draft — ordering matters here.
  const lastPlayerMsgForCheck = [...chat].reverse().find(m=>m.role==='user');
  const lastPlayerTextForCheck = lastPlayerMsgForCheck ? (lastPlayerMsgForCheck.text || '') : '';
  queueWorldOp(world.id, async ()=>{
    try{
      const playerAskedForSkip = skipDaysInLine(lastPlayerTextForCheck) > 0;
      const phraseFlaggedSkip = (mode !== 'forward') && !playerAskedForSkip && skipDaysInLine(reply) > 0;
      const violations = await detectStoryRuleViolations(reply, lastPlayerTextForCheck);
      const timeSkipFlagged = (mode !== 'forward') && !playerAskedForSkip && (phraseFlaggedSkip || violations.timeSkip);
      const systemGrantFlagged = violations.systemGrant;
      if(timeSkipFlagged || systemGrantFlagged){
        let correctionNote = `\n\nYour previous attempt broke the following rule(s) — here is the reply you must NOT repeat:\n"""\n${reply}\n"""\n`;
        if(timeSkipFlagged) correctionNote += `\n- NO SELF-INITIATED TIME SKIPS: it narrated (or implied) time passing, a night passing, or a new day/morning starting even though the player's own message didn't ask for that.`;
        if(systemGrantFlagged) correctionNote += `\n- NO SELF-INITIATED SYSTEM/GAME-INTERFACE EVENTS: it had the System/interface hand out a reward, item, or notification even though the player's own message didn't ask for, check, or open it this turn.`;
        correctionNote += `\n\nWrite a new reply instead that stays inside the exact same moment as "Story so far" above and responds to what the player actually did or said — with no time-skip and no System/interface activity of any kind, stated or implied, unless the player's own message explicitly asked for it this turn.`;
        try{
          const fixedReply = await askAI(worldSystemPrompt(world, memory, panel), `Story so far:\n${recentLog}\n\n${instruction}${finalReminder}${correctionNote}`);
          if(fixedReply){
            reply = fixedReply; // one retry only — accept it as-is rather than looping further
            if(chat[msgIndex]) chat[msgIndex].text = fixedReply;
            await saveChat(world.id, chat);
            updateRenderedMsgText(msgIndex, fixedReply);
          }
        }catch(e){ console.warn('[story-rule retry] failed, keeping original reply', e); }
      }
    }catch(e){ console.warn('[story-rule background check] failed, leaving reply as-is', e); }
    await updateMemory(world); await updatePanel(world, panelOpts);
  });
}
...
// ---------- media drawer: open/close ----------
// Restored (see the long-press wiring near sendMessage/els.sendBtn at the bottom of this
// file for how it's triggered without breaking a normal tap-to-send).
function openMediaDrawer(){
  els.mediaDrawerOverlay.style.display = 'block';
  // Force a reflow before adding the class so the slide-up transition actually animates
  // instead of the drawer just snapping open already-in-place.
  void els.mediaDrawer.offsetWidth;
...
- "Holdings" (real property — land, houses, shops) also appears on the sheet, but is OFF-LIMITS to you entirely: never propose an addition, change, or removal there, no matter what the story shows the character buying, inheriting, losing, or building. It's populated only once, from the world's own setup text, before the story begins — treat it as read-only.
...
- CURRENCY lives in "Finances" (a permanent kv category, always on the sheet, but empty until the story actually gives the character money — never invent a starting balance for a currency that hasn't been earned, found, or given yet). Every value must be written as the amount followed by its unit, spelled out the way the story names it, not a bare number — e.g. {"Ryo":"120,200 ryo"}, {"Dollars":"299 dollars"}, {"Gold":"1 g of gold"}, {"Copper":"200 copper coins"}. Reuse the same key for every reference to the SAME currency/unit; never spin up a separate category for money, and never track the identical currency under two differently-worded keys just to rename it. This does NOT mean collapsing different units into one entry — see the next two rules.
- METAL CURRENCY — COIN FORM VS WEIGHT FORM: when the story's money is metal-based, there are two ways it gets counted, and which one applies depends on the metal:
  - Coin form (a discrete count) applies to any metal the story treats as coinage: "<amount> gold coins", "<amount> silver coins", "<amount> bronze coins", "<amount> copper coins", "<amount> iron coins", "<amount> platinum coins" — or, if the setting instead counts gold/silver in taels ("one tael of silver", "three taels of gold"), use that phrasing instead of "coins" for that metal, matching however the story actually names the unit.
  - Weight form (grams) is reserved for gold and silver ONLY — "1 gram of gold", "2 grams of silver". Never write bronze, copper, or iron by weight; those metals are coin-form only.
  - Always follow whichever form the story's own narration/setting actually uses for that metal, and stay consistent with it for the rest of the story once established.
- SEPARATE CURRENCIES/UNITS NEVER MERGE INTO ONE TOTAL: if the story has the character holding more than one kind of money at once — e.g. "1 gram of gold", "10 gold coins", and "100 dollars" all in the same story — each gets its OWN kv key in Finances ("Gold (weight)":"1 gram of gold", "Gold Coins":"10 gold coins", "Dollars":"100 dollars") and its own running amount, tracked completely independently. Never add them together into a single combined figure, never convert one into another unless the log itself explicitly shows an in-story exchange/conversion happening, and never assume gold-by-weight and gold-coins are "the same entry" just because they're both gold — a coin count and a raw weight are different things being tracked. The player can spend any one of them on its own, several together in the same transaction, or none at all — each currency/unit's balance only ever changes when the log grounds a change to THAT specific one.
- EXCHANGES/TRADES/CONVERSIONS: an exchange ("I exchange 100 for gold", "I trade my dagger for 50 ryo", "I swap 200 credits for a room key", "I sell my gold for cash", "I sold 10 silver coins for 50 dollars") is a completed transaction exactly like a purchase once the log shows it actually happening — apply the same rule above. Every exchange has two sides, and BOTH must be output together in the same response: whatever is given up decreases (or leaves Inventory entirely if a whole item was traded away) and whatever is received increases or is added — never record one side without the other. This holds no matter which two things are being exchanged: currency-for-currency, currency-for-item, item-for-currency, or item-for-item. Selling is just an exchange in the other direction — the currency/item sold decreases (or leaves Inventory) exactly the same way a purchase would decrease the currency spent, while whatever was received for it increases — never record a sale as only a gain on one side with no matching decrease on the other.
...
- STATUS: a single free-text field holding the character's current condition — physical, health, or notable emotional/mental state — kept short (one sentence, or a few comma-separated notes). This is the one category with no other guard checking it, so it is entirely your responsibility to actively check it every single turn, the same as every other category above — never skip it just because nothing feels dramatic enough. Update it whenever the log shows any of these actually happening on-page:
  - An injury, wound, illness, or any change in physical condition (hurt, healing, exhausted, poisoned, bleeding, recovered, etc.)
  - A notable shift in mood or mental state that the narration itself shows (shaken, furious, elated, grieving, terrified, calm again after being upset, etc.) — not just a passing adjective in dialogue, but something the story is actually depicting the character experiencing right now.
  - A temporary effect starting or ending (a buff/debuff, a curse, being under the influence of something, a technique's side effect, exhaustion from overusing a power, etc.)
  - A significant change in circumstance that colors the character's current state (in hiding, on the run, under suspicion, recovering at home, etc.) — but only once the log actually shows that becoming true, never from a plan or intention.
  Do NOT invent a status just to have one, and do not restate the same unchanged condition turn after turn — only output "Status" when something above has genuinely changed since the last update. If the character's condition hasn't shifted, simply omit "Status" from your output entirely rather than repeating the old value.
...
async function updatePanel(world, opts){
  ...
  try{
    const bgModel = await getGeminiBgModel();
    const raw = await askAIWithRetry(PANEL_SYS_PROMPT, prompt, bgModel);
    let data = extractJsonObject(raw);
    const lastPlayerMsg = [...chat].reverse().find(m=>m.role==='user');
    // ---------- backstop: background pass missed an obvious practice/study phrase ----------
    // guardSkillProgress (script_saal.js) only ever VALIDATES a Learning change the background
    // pass already proposed — it can't create one out of nothing. Unlike Timeline (which has
    // ensureTimelineDayFallback as a deterministic backstop), Learning has no such fallback, so
    // when this pass simply doesn't notice a practice phrase was worth logging — plain model
    // inconsistency, not a rule — nothing gets proposed and the guard has nothing to check. This
    // gives it one narrow, targeted second look specifically at that gap, rather than trusting
    // the first pass caught everything. PRACTICE_TRIGGER_RE is defined in script_saal.js — safe
    // to reference here, all files share one global scope.
    const lastPlayerText = lastPlayerMsg ? (lastPlayerMsg.text || '') : '';
    if(PRACTICE_TRIGGER_RE.test(lastPlayerText)){
      const proposedLearningChange = data && data.categories && Object.entries(data.categories)
        .some(([catName, catUpdate]) => /^learning$/i.test(catName) && catUpdate && catUpdate.kv && Object.keys(catUpdate.kv).length > 0);
      if(!proposedLearningChange){
        try{
          const followupPrompt = `${prompt}\n\nDouble-check one thing before finalizing: the player's own latest message was:\n"""${lastPlayerText}"""\nThat message describes them studying, training, or practicing something. If the recent story log actually shows them beginning or continuing real training on a specific skill/technique/spell that isn't already fully mastered in "Skills & Abilities", make sure your JSON output includes that as a "Learning" kv update (a new entry at "10%" if this is the first time, or the existing entry bumped by the normal step if it already exists) — output the SAME JSON shape as before, this time including that Learning update if it genuinely belongs. If nothing in the log actually names a specific trainable skill, or that skill is already fully mastered, it's correct to leave Learning out entirely — only add it if it genuinely applies.`;
          const raw2 = await askAIWithRetry(PANEL_SYS_PROMPT, followupPrompt, bgModel);
          const data2 = extractJsonObject(raw2);
          if(data2 && data2.categories){
            for(const [catName, catUpdate] of Object.entries(data2.categories)){
              if(!/^learning$/i.test(catName) || !catUpdate || !catUpdate.kv) continue;
              data = data || {categories:{}};
              data.categories = data.categories || {};
              data.categories[catName] = data.categories[catName] || {};
              data.categories[catName].kv = Object.assign({}, data.categories[catName].kv || {}, catUpdate.kv);
            }
          }
        }catch(e){ console.warn('[learning retry] failed, proceeding without it', e); }
      }
    }
    // FIX(10)/script_finance.js: the currency-decrease confirmation used to check ONLY the
    // single last player message, even though `recent` (and thus the grounding checks) spans
    // the last 10 messages / ~5 exchanges. That mismatch let a real spend confirmed 1-2 turns
    // ago get wrongly blocked if the background model only got around to applying it later.
    // Now checks every player message in that same 10-message window, same approach the resync
    // path (resyncMemoryAndPanel/playerTextInWindow below) already used.
    const recentPlayerText = chat.slice(-10).filter(m=>m.role==='user').map(m=>m.text||'').join('\n');
    // FIX(23)/script_finance.js: guardCurrencyDecreases also gets each player message's
    // absolute chat-array index here (not just the flattened string above), so it can tell a
    // genuinely new spend confirmation apart from the same one still sitting in the 10-message
    // window from a turn or two ago — see the ledger comment on guardCurrencyDecreases itself.
    const recentWindowStart = Math.max(0, chat.length - 10);
    const recentPlayerMessages = chat.slice(-10)
      .map((m, i) => ({ index: recentWindowStart + i, role: m.role, text: m.text || '' }))
      .filter(m => m.role === 'user');
    guardCurrencyDecreases(data, panel, recentPlayerMessages, recent);
    // FIX(4)/script_finance.js: guardCurrencyIncreases now also takes playerText (4th arg) so
    // it can confirm a sell/trade/swap-for-money gain when paired with an actual Inventory
    // decrease this same turn.
    guardCurrencyIncreases(data, panel, recent, recentPlayerText);
    guardCurrencyRenameBypass(data, panel, recent);
    // Holdings (script_finance.js): lets a new entry through only when BOTH check out — a
    // grounded acquisition verb + property noun somewhere in recent+recentPlayerText, AND
    // recentPlayerText itself names a tracked currency + quantity that fits the current
    // balance (holdingAffordabilityIssue). An edit or removal of an existing entry is still
    // always blocked regardless.
    guardHoldingsAcquisition(data, panel, recent, recentPlayerText);
    ...
    // ensureTimelineDayFallback now reads ONLY the player's own latest message — not the
    // story's reply. The reply is free-form narration that often restates a span more than
    // once (flavor dialogue repeating a number, "a week ago" backstory asides, etc.), and any
    // of that inflates a fallback that's meant to just reflect what the player actually typed.
    // The player's own bubble is the one unambiguous source: they either wrote a time-skip
    // phrase or they didn't, once.
    ensureTimelineDayFallback(data, panel, lastPlayerMsg ? lastPlayerMsg.text : '', world.id);
...
async function resyncMemoryAndPanel(world, chat){
  ...
  try{
    ...
    // FIX(4)/script_finance.js: playerTextInWindow passed here too, for the same
    // sell/trade/swap-for-money confirmation path used in the normal per-turn flow above.
    guardCurrencyIncreases(data, oldPanel, convo, playerTextInWindow);
    guardCurrencyRenameBypass(data, oldPanel, convo);
    // Holdings (script_finance.js): same rule as the other call sites on a rewind/regenerate
    // resync — needs a grounded acquisition verb + property noun in convo/playerTextInWindow,
    // AND playerTextInWindow itself naming an affordable tracked currency + quantity. An
    // edit/removal of an entry already on the sheet is still always stripped.
    guardHoldingsAcquisition(data, oldPanel, convo, playerTextInWindow);
...
// Deliberately silent now — this used to render a ⚠️ warning (plus a Retry button) directly in
// the story log whenever a background sync (memory/panel/Letter of Records) failed. On a flaky
// connection that fired often enough to become its own disruption, breaking up the story with
// red warning text mid-scene. The underlying retry logic in updatePanel/updateMemory etc. is
// untouched and still runs exactly as before — this only removes the on-screen notice. The
// real trade-off: a background update that fails now fails completely silently. If an item/
// stat mentioned in the story stops showing up on the sheet, there's no more on-screen sign
// anything went wrong — mentioning the thing again naturally in a later message is still the
// way to give the next sync pass a fresh chance to catch it (same as before this warning
// existed at all).
function showBackgroundWarning(worldId, label, err){
  console.error(`[${label} failed]`, err);
}

// FIX(restore-media-longpress): sendMessage's body is now sendUserTurn({media}) so the
// long-press-to-attach flow below can push the exact same message shape (text + optional
// media) through the exact same render/claim-check/save/continue pipeline, instead of a
// second, drifting copy of it.
//
// NOTE: the claim check runs BEFORE anything is rendered, on request — it can make sending
// feel stuck while it scans (especially on a slow connection), but the player asked to keep
// it exactly this way: every message actually gets scanned before it ever reaches the story
// or the screen, rather than flashing on screen and possibly getting retracted a moment
// later. A prior version of this function rendered optimistically and rolled a message back
// on a failed scan instead — deliberately reverted back to blocking first.
async function sendUserTurn(opts){
  opts = opts || {};
  const media = opts.media || null;
  if(isSending) return;
  const val = els.textInput.value.trim();
  if(!val && !media) return; // nothing typed and nothing attached — nothing to send
  // Guard set immediately, before any await — otherwise a rapid double-tap can slip
  // through the gap while world/chat are still being fetched and fire twice.
  isSending = true; els.sendBtn.disabled = true;
  const world = await getWorld(state.chattingId);
  ...
  // A caption-less attachment has no claim to check (nothing typed to verify against the
  // sheet) — skip Layer 2's AI round-trip entirely rather than spending a call on an empty
  // string just to get a guaranteed-null result back.
  if(val && !isSystemTrigger(val)){
    ...
    if(claimIssue){
      ...
      // player can just edit and resend — nothing was ever shown, so there's nothing to
      // retract.
  ...
  document.querySelectorAll('.msg-actions').forEach(a=>a.remove());
  document.querySelectorAll('.sys-tools-wrap').forEach(a=>a.remove());
  const msg = {role:'user', text:val, ts:Date.now()};
...
async function sendMessage(){
  if(!els.textInput.value.trim()) return;
  await sendUserTurn();
}

// ---------- send-button long-press: attach media (photo/gif/video) ----------
// Restores the previously-removed gesture. THE PITFALL THAT BROKE IT LAST TIME (documented
// at the top of this file): once the hold threshold is reached and the drawer opens, the
// finger/mouse still has to lift off els.sendBtn — and that release fires an ordinary
// 'click' on it, same element, same as any tap. Without suppressing that one synthetic
// click, every long-press would ALSO immediately fire sendMessage() the instant the drawer
// opened. Handled here with a one-shot flag: the hold-timer sets it right before opening the
// drawer, and the click handler below consumes-and-clears it instead of sending — a normal,
// quick tap never touches the timer long enough to set it, so ordinary tap-to-send is
// completely unaffected.
const LONG_PRESS_MS = 450;
const LONG_PRESS_MOVE_TOLERANCE = 12; // px of finger drift before a hold is cancelled, not fired
let _longPressTimer = null;
let _longPressOrigin = null;
let _suppressNextSendClick = false;
function cancelLongPress(){
  if(_longPressTimer){ clearTimeout(_longPressTimer); _longPressTimer = null; }
  _longPressOrigin = null;
}
els.sendBtn.addEventListener('pointerdown', (e)=>{
  if(e.pointerType === 'mouse' && e.button !== 0) return; // ignore right/middle click
  _longPressOrigin = { x: e.clientX, y: e.clientY };
  clearTimeout(_longPressTimer);
  _longPressTimer = setTimeout(()=>{
    _longPressTimer = null;
    _suppressNextSendClick = true;
    openMediaDrawer();
  }, LONG_PRESS_MS);
});
els.sendBtn.addEventListener('pointermove', (e)=>{
  if(!_longPressOrigin) return;
  const dx = e.clientX - _longPressOrigin.x, dy = e.clientY - _longPressOrigin.y;
  if(Math.hypot(dx, dy) > LONG_PRESS_MOVE_TOLERANCE) cancelLongPress();
});
els.sendBtn.addEventListener('pointerup', cancelLongPress);
els.sendBtn.addEventListener('pointercancel', cancelLongPress);
els.sendBtn.addEventListener('pointerleave', cancelLongPress);

els.mediaDrawerOverlay.onclick = closeMediaDrawer;
let _pendingMediaKind = 'image';
els.mediaPhotoBtn.onclick = ()=>{ _pendingMediaKind='image'; els.mediaFileInput.accept='image/*'; closeMediaDrawer(); els.mediaFileInput.click(); };
els.mediaGifBtn.onclick   = ()=>{ _pendingMediaKind='gif';   els.mediaFileInput.accept='image/gif'; closeMediaDrawer(); els.mediaFileInput.click(); };
els.mediaVideoBtn.onclick = ()=>{ _pendingMediaKind='video'; els.mediaFileInput.accept='video/*'; closeMediaDrawer(); els.mediaFileInput.click(); };

function showMediaAttachWarning(msg){
  const w = document.createElement('div'); w.className='warn'; w.textContent = `⚠️ ${msg}`;
  els.log.appendChild(w); scrollLogToBottom(); pinToBottomAfterRender();
}
const MAX_GIF_BYTES = 8 * 1024 * 1024;
const MAX_VIDEO_BYTES = 15 * 1024 * 1024;
els.mediaFileInput.onchange = async ()=>{
  const file = els.mediaFileInput.files[0];
  els.mediaFileInput.value = ''; // reset so picking the exact same file twice still fires 'change'
  if(!file) return;
  const kind = _pendingMediaKind;
  if(kind === 'gif' && file.size > MAX_GIF_BYTES){
    showMediaAttachWarning(`That GIF is too large to attach (max ${(MAX_GIF_BYTES/1024/1024)|0}MB).`);
    return;
  }
  if(kind === 'video' && file.size > MAX_VIDEO_BYTES){
    showMediaAttachWarning(`That video is too large to attach (max ${(MAX_VIDEO_BYTES/1024/1024)|0}MB).`);
    return;
  }
  let url;
  try{
    // Photos get compressed the same way every other photo upload in this app already does
    // (resizeImageToDataUrl, index.html). GIFs and videos deliberately skip that path — canvas-
    // drawing a GIF flattens it to a single static frame (killing the animation), and canvas
    // can't touch video at all — so both are just read directly as a data URL instead.
    if(kind === 'image') url = await resizeImageToDataUrl(file, 900, 0.75);
    else url = await new Promise((resolve, reject)=>{
    ...
    showMediaAttachWarning('Could not read that file — try a different one.');
    return;
  }
  await sendUserTurn({ media: { kind: kind === 'video' ? 'video' : 'image', url } });
};

els.sendBtn.onclick = (e)=>{
  if(_suppressNextSendClick){ _suppressNextSendClick = false; return; }
  sendMessage();
};
