...
// consulted for the System-grant path (SYSTEM_GRANT_RE) above — it can never satisfy the plain
// ITEM_GAIN_TRIGGER_RE/abilityMentionedNear checks, which stay scoped to playerText exactly as
// before. Passing '' or omitting it entirely reproduces the old player-only behavior unchanged.
function itemGainConfirmed(playerText, itemLabel, panel, recentLogText){
  return itemMentionedNear(playerText, itemLabel, ITEM_GAIN_TRIGGER_RE)
    ...
    || (recentLogText ? itemMentionedNear(recentLogText, itemLabel, SYSTEM_GRANT_RE) : false);
}
...
const SYSTEM_GRANT_RE = /\[?\s*system\s*\]?\s*(?:notification)?\s*[:\-]|\bthe\s+system\b[^.\n]{0,40}\b(?:grants?|gives?|awards?|notifies?|announces?)\b|\byou\s+have\s+(?:\w+\s+){0,2}(?:obtained|received|acquired|gained|been\s+(?:given|awarded|granted))\b|\byou\s+(?:successfully\s+)?(?:obtain|receive|acquire|gain)(?:ed)?\b[^.\n]{0,15}:|\b(?:loot|quest\s+reward|item)\s*(?:obtained|acquired|received)\b|\bnew\s+item\s+added\b|\badded\s+to\s+(?:your\s+)?inventory\b|\bstatus\s+window\b|\bnotification\s+chimes?\b/i;
...
function renderEquipCompassHtml(items, ids, photos){
  const { bySlot, disabledSlots } = computeEquipSlots(items, ids);
  const cells = EQUIP_SLOT_RULES.map(rule=>{
    ...
    const iconInner = photoUrl
      ...
      : (item ? escapeHtml(invSlotGlyph(item.name)) : (isDisabled ? '🚫' : rule.icon));
    const cellClass = `equip-cell${item ? ' is-filled' : ''}${isDisabled ? ' is-disabled' : ''}`;
