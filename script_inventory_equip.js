...
// ##############################################################################
// SECTION 2B — INVENTORY LORE SEED
// seedInventoryFromLore — same one-time, !hadStory-gated pattern as script_finance.js's
// seedFinancesFromLore/seedHoldingsFromLore: pulls whatever starting items the player wrote
// directly into the world's own setup text and writes them onto the sheet ONCE, at world
// creation. Unlike Finances/Holdings, Inventory stays fully live through the story afterward —
// every guard above this section keeps policing it turn to turn — so this only closes the one
// gap none of those per-turn guards ever could: an item declared in the lore itself, before any
// story text exists yet for a guard like guardStackableItems to ground a gain against. Without
// this, typing "you start with 3 health potions and a rusty kunai" into World Lore could never
// reach the sheet on its own, the same problem seedFinancesFromLore closed for a starting
// balance and seedHoldingsFromLore closed for starting property.
// ##############################################################################
async function seedInventoryFromLore(world){
  if(!world || !world.lore || !world.lore.trim()) return;
  const panel = await getPanel(world.id);
  const invCat = panel.categories['Inventory'];
  if(invCat && Array.isArray(invCat.data) && invCat.data.length > 0) return;
  const prompt = `World setup text:\n${world.lore}\n\nDoes this text explicitly state that the player character starts the story already carrying/owning any specific items \u2014 weapons, tools, supplies, gear, clothing, or similar? Only count items explicitly stated as already possessed by the player character right at the start of the story \u2014 not items merely for sale nearby, owned by someone else, or that might be found/acquired later.\n\nIf yes, output ONLY a raw JSON object, no preamble, no markdown fences, in this exact shape: {"categories":{"Inventory":{"list_add":["<qty> <item name>", "..."]}}} \u2014 one entry per distinct item, quantity first (use "1" if the text gives no count), e.g. "1 Rusty Kunai", "3 Health Potions", "1 Traveling Cloak". Append " \u2014 Equipped" to an entry ONLY if the text explicitly says the character is already wearing/wielding/carrying it on their person at the very start (e.g. "1 Traveling Cloak \u2014 Equipped").\n\nIf no starting items are explicitly stated anywhere in the text, output exactly {"categories":{}}.`;
  try{
    const bgModel = await getGeminiBgModel();
    const raw = await askAIWithRetry('You extract a single fact — items explicitly stated as already owned by the player character, if any — from a piece of world-setup text. Output only the raw JSON object described, nothing else.', prompt, bgModel);
    const data = extractJsonObject(raw);
    const listAdd = data && data.categories && data.categories['Inventory'] && data.categories['Inventory'].list_add;
    if(!Array.isArray(listAdd) || listAdd.length === 0) return;
    // Re-fetch and re-check right before writing, same reasoning as seedFinancesFromLore/
    // seedHoldingsFromLore: the AI call above took real time, and getPanel's own backfill/
    // migration passes can still have touched the panel meanwhile.
    const freshPanel = await getPanel(world.id);
    const freshInv = freshPanel.categories['Inventory'];
    if(freshInv && Array.isArray(freshInv.data) && freshInv.data.length > 0) return;
    mergePanelUpdate(freshPanel, { categories: { Inventory: { list_add: listAdd } } });
    await savePanel(world.id, freshPanel);
  }catch(e){ console.warn('[inventory lore seed] failed', e); }
}
