...
function renderEquipCompassHtml(items, ids, photos, equipHub){
  ...
  const accessoriesHtml = accessories.length
    ...
    : '';
  const topUrl = equipHub && equipHub.top;
  const bottomUrl = equipHub && equipHub.bottom;
  const topInner = topUrl
    ? `<img class="equip-hub-img" id="equipHubTopIcon" src="${escapeHtml(topUrl)}">`
    : `<span class="equip-hub-placeholder" id="equipHubTopIcon">+ Add image</span>`;
  const bottomInner = bottomUrl
    ? `<img class="equip-hub-img" id="equipHubBottomPlaceholder" src="${escapeHtml(bottomUrl)}">`
    : `<span class="equip-hub-placeholder" id="equipHubBottomPlaceholder">+ Add image</span>`;
  const hub = `<div class="equip-hub-top" onclick="document.getElementById('equipHubTopFile').click()">${topInner}</div><div class="equip-hub-bottom" onclick="document.getElementById('equipHubBottomFile').click()">${bottomInner}</div>`;
  return `<div class="equip-compass-wrap">
    <div class="equip-hub-row">${hub}</div>
    <div class="equip-legend">${sectionsHtml}</div>
    ${accessoriesHtml}
  </div>`;
}

// ---------- equip-box tap-to-upload handlers (moved from script_chatroom.js) ----------
// Tap-to-upload handlers for the two frozen equip-hub sections (top figure slot, bottom blank
// slot) rendered by renderEquipCompassHtml above. The picker inputs live once in the page; the
// target elements are looked up fresh on each change since the equip section's inner HTML is
// re-rendered every time the Holding tab is opened. Plain top-level IIFE, same as before the
// move — global scope, so it still wires up these listeners once the page loads.
(function initEquipHubPickers(){
  ...
  topInput.addEventListener('change', (e)=>{
    ...
    e.target.value = '';
    if(!file || !state.chattingId) return;
    const reader = new FileReader();
    reader.onload = async () => {
      // Persisted via setEquipHubPhoto (panel.equipHub → IndexedDB) rather than just swapped
      // into the DOM, so it survives a repaint/tab-switch/reload instead of vanishing the next
      // time renderEquipCompassHtml runs. savePanel's own live-refresh repaints this section
      // right after, so no manual DOM update is needed here.
      await setEquipHubPhoto(state.chattingId, 'top', reader.result);
    };
    reader.readAsDataURL(file);
  });
  ...
  bottomInput.addEventListener('change', (e)=>{
    ...
    const reader = new FileReader();
    reader.onload = async () => {
      await setEquipHubPhoto(state.chattingId, 'bottom', reader.result);
    };
    reader.readAsDataURL(file);
  });
})();
