/* ================================================================================
   ...
   pre-send claim checks (checkClaimAgainstRecords's Layer 1 deterministic
   checkCurrencyClaim/checkInventoryClaim/checkAbilityClaim, plus Layer 2's
   check*ClaimFromAI functions), the merge engine (key matching + mergePanelUpdate —
   generic, shared by every category), panel rendering (HTML output,
   relationshipShortLabel), and the info modal (CAT_INFO/getCatInfo).
   ...
   file's merge engine, panel renderer, and CAT_INFO modal are still the shared,
   generic machinery those categories render and describe through, so they call
   into functions here (findExistingKey, mergePanelUpdate, panelToText,
   renderPanelHtml, genId, etc.) same as script_chatroom.js does.
...
// all in script_fieb.js (Finances, Inventory) and script_tsal.js (Timeline,
// Scheduled Events, Skills & Abilities) — see the MOVED banners below.
// ================================================================================
...
// scheme and its one-time legacy-id upgrade) now live in script_fieb.js
...
// script_fieb.js (SECTION 2 — INVENTORY), right next to the
...
// mergeStrayInventoryCategories now lives in script_fieb.js
...
// migrateSkillCategoryNames now lives in script_tsal.js
// (see SKILLS & ABILITIES — LEARNING GUARDS section there). Still called from getPanel below,
...
// now live in script_tsal.js (SKILLS & ABILITIES — LEARNING
// GUARDS section). Still called from getPanel above and from mergePanelUpdate below, same as
// before the split.

// ================= FINANCES & INVENTORY — MOVED =================
...
// script_fieb.js. Load that file alongside this one;
...
// script_tsal.js. Load that file alongside this one;
...
// lives in script_fieb.js. Load that file alongside
...
// lives in script_tsal.js. Load that file alongside
...
// seedSkillsFromLore — MOVED — now lives in script_tsal.js
// (SKILLS & ABILITIES — LEARNING GUARDS section). Still called from the world-save handler
// above (queueWorldOp(id, ...)), same as before the split.

// ================= PRE-SEND CLAIM CHECKS =================
// The entire pre-send claim-check pipeline: Layer 1 deterministic regex/math checks
// (checkCurrencyClaim, checkInventoryClaim, checkAbilityClaim), Layer 2's AI read step
// for disguised/implied claims (readClaimsFromInput), their code-side checkers
// (check*ClaimFromAI), and checkClaimAgainstRecords tying both together. Still called
// from sendMessage() in Section 2 above, same as before the move.
...
// now live in script_fieb.js, right next to the rest of the Inventory
...
// script_fieb.js (SECTION 2 — INVENTORY). Still called from
...
// script_fieb.js, right next to the equip-box HTML they
