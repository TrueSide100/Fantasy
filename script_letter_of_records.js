/* ================================================================================
   ...
   checkCurrencyClaim/checkInventoryClaim, Layer 2's readClaimsFromInput and its
   checkInventoryClaimFromAI/checkCurrencyClaimFromAI checkers), the merge engine
   ...
   getCatInfo). The ability/skill half of that same pipeline — checkAbilityClaim,
   checkAbilityClaimFromAI, and their shared claim-phrase helpers — now lives in
   script_saal.js; checkClaimAgainstRecords below still calls straight into them
   (global scope, so load order doesn't matter).
...
// Learning, plus the ability/skill half of the pre-send claim checks) — see the MOVED
// banners below.
// ================================================================================
...
// The pre-send claim-check pipeline lives here except for its ability/skill half
// (checkAbilityClaim, checkAbilityClaimFromAI — moved to script_saal.js, see the
// two MOVED notes further down for exactly where). Still here: Layer 1's other
// deterministic regex/math checks (checkCurrencyClaim, checkInventoryClaim),
// Layer 2's AI read step for disguised/implied claims (readClaimsFromInput) and
// its non-ability code-side checkers (checkInventoryClaimFromAI,
// checkCurrencyClaimFromAI), and checkClaimAgainstRecords, which ties every layer
// together and calls straight into the moved ability functions same as before
// (global scope). Still called from sendMessage() in Section 2 above, same as
// before the move.
