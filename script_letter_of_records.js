/* ================================================================================
   ...
   pre-send claim-check pipeline (checkClaimAgainstRecords, checkScheduleSkipClaim,
   Layer 2's shared readClaimsFromInput AI read step), the merge engine
   ...
   the Finance guards; the ability/skill half — prechecker (fully manual,
   regex-only — no AI checker for abilities anymore) and its shared claim-phrase
   helpers — lives in script_saal.js; the inventory half — checkInventoryClaim,
   checkInventoryClaimFromAI, parseInventoryEntry, textMentionsItem,
   ACQUIRE_VERBS_SRC, and claimLooksAcquired — lives in script_inventory.js;
   checkClaimAgainstRecords below still calls straight into all of them (global
   scope, so load order doesn't matter). The info modal's own Skills &
...
function defaultPanel(){
  // FIX(23)/script_finance.js: currencyGroundLedger records, per normalized currency key,
  // which player-message chat-array indices have already been used by guardCurrencyDecreases
  // to ground a currency decrease. Persisted here (not just held in memory) for the same
  // reason numMap/photos/equipHub are — it has to survive getPanel()'s sanitizePanel() pass
  // on every load, or it would silently reset and the bug it exists to fix would come back.
  return { numSeq:0, numMap:{}, photos:{}, equipHub:{ top:null, bottom:null }, currencyGroundLedger:{}, categories: {
    'Identity': { type:'kv', data:{}, ids:{} },
    'Finances': { type:'kv', data:{}, ids:{} },
    // Holdings — property (land/houses/shops), NOT part of the normal per-turn Finances/
    // Inventory tracking. See guardHoldingsAcquisition (script_finance.js): a story turn can
    // ADD a new entry here, but only when the recent text grounds an actual acquisition (bought/
    // inherited/given/... a house/shop/land/...) — it can never change or remove an entry
    // already on the sheet. Initial ownership stated in the world's own setup text is instead
    // seeded once via seedHoldingsFromLore (also script_finance.js) at world creation.
    // A plain list, same shape as Milestones — each entry is a short property description.
    'Holdings': { type:'list', data:[], ids:[] },
...
function migrateOldPanel(old){
  ...
  // FIX(23): currencyGroundLedger starts empty for a migrated panel — there's no prior
  // grounding history to preserve, same as a brand-new one.
  const panel = { numSeq:0, numMap:{}, photos:{}, equipHub:{ top:null, bottom:null }, currencyGroundLedger:{}, categories: {
...
function sanitizePanel(p){
  ...
  // FIX(23): currencyGroundLedger passed through here too, same as numMap/photos/equipHub —
  // without this it would get silently dropped on every getPanel() call (sanitizePanel only
  // ever copies fields it explicitly knows about), resetting the ledger every load and
  // letting the same stale spend-confirmation re-validate decreases again.
  const out = { categories:{}, numSeq: typeof p.numSeq==='number' ? p.numSeq : 0, numMap: (p.numMap && typeof p.numMap==='object') ? p.numMap : {}, photos: (p.photos && typeof p.photos==='object') ? p.photos : {}, equipHub: (p.equipHub && typeof p.equipHub==='object') ? { top: p.equipHub.top || null, bottom: p.equipHub.bottom || null } : { top:null, bottom:null }, currencyGroundLedger: (p.currencyGroundLedger && typeof p.currencyGroundLedger==='object') ? p.currencyGroundLedger : {} };
...
async function savePanel(id, panel){
  ...
  if(state.chattingId === id){
    ...
    if(els.econModal.style.display === 'flex') await paintEconModal();
  }
}
...
// its ability/skill half (prechecker — moved to script_saal.js, fully manual/
// regex-only there now, no AI checker; see the MOVED notes further down for exactly
// where it went). Still here: Layer 1's deterministic checkInventoryClaim, Layer 2's AI
...
// ---------- inventory claim checker (deterministic + AI-claim backstop) — MOVED ----------
// parseInventoryEntry, textMentionsItem, ACQUIRE_VERBS_SRC, checkInventoryClaim,
// claimLooksAcquired, and checkInventoryClaimFromAI now live in script_inventory.js
// (INVENTORY — PRE-SEND CLAIM CHECK section), right next to splitItemEntry and the rest
// of the Inventory guards, since all of these exist only to police inventory claims —
// same reasoning as the currency/ability checkers moving to script_finance.js/
// script_saal.js. Still called from checkClaimAgainstRecords below the same way (global
// scope, so load order doesn't matter), same as before the split.

// ---------- ability/item/power claim checker — MOVED ----------
// prechecker (the deterministic regex check) and its shared claim-phrase helpers
...
// (SKILLS & ABILITIES — PRE-SEND CLAIM CHECK section). checkAbilityClaimFromAI was
// removed entirely (no longer exists anywhere) — abilities are no longer covered by
// the Layer 2 AI-read step below at all, only by Layer 1's prechecker.
// checkCurrencyClaim/checkInventoryClaim directly above are unrelated and
...
Do NOT extract ability/power claims (e.g. "I use my Sharingan") — those are checked elsewhere, purely by code, and are not part of your job.
...
// — PRE-SEND CLAIM CHECK section), alongside prechecker and their shared
// claim-phrase helpers. Still called from checkClaimAgainstRecords below the same way
// (global scope), same as before the split.

// claimLooksAcquired / checkInventoryClaimFromAI — MOVED — now live in script_inventory.js
// (INVENTORY — PRE-SEND CLAIM CHECK section), alongside checkInventoryClaim and their
// shared helpers. Still called from checkClaimAgainstRecords below the same way (global
// scope), same as before the split.
...
async function checkClaimAgainstRecords(world, userText){
  ...
  const abilityIssue = prechecker(userText, panel);
  ...
  try{
    ...
    // Reverted back to this on request: every message is meant to actually go through this
    // scan before it reaches the story, not slip through unchecked just because the scan
    // itself couldn't complete — a slower send is preferred here over a skipped one.
    return "Couldn't scan this message against your sheet (AI read step failed) — check your connection/API key and try again.";
  }
  for(const c of claims){
    let issue = null;
    // 'ability' claims are deliberately NOT checked here anymore — checkAbilityClaimFromAI
    // was removed from script_saal.js on request, so Skills & Abilities claims are now caught
    // ONLY by Layer 1's prechecker above (fully manual, regex-only, no AI involved at
    // any point for abilities). A disguised ability claim that Layer 1's regex genuinely can't
    // catch will no longer be caught at all — that tradeoff was explicit and intentional.
    if(c.type === 'item') issue = checkInventoryClaimFromAI(c, panel, userText);
...
function mergePanelUpdate(panel, update){
  ...
  for(const [rawName, catUpdate] of Object.entries(update.categories)){
    ...
    if(cat.type==='list'){
      ...
      // BUG FIX (orphaned photos on story-driven deletion): the long-press action sheet's
      // Delete button already clears panel.photos[id] for the item it removes, but a story-
      // driven removal (the AI narrating an item thrown away/lost/given away/sold, processed
      // as a list_remove above) never went through that same cleanup — the stored snapshot for
      // that id just sat in panel.photos forever with nothing left in Inventory to show it on.
      // Whatever's still in removedThisTurn at this point is genuinely gone (anything that was
      // really just a correction — a status/qty/wording rewrite — already had its id reused
      // and spliced back out of this array above), so its photo, if any, is cleared too.
      if(removedThisTurn.length && panel.photos){
        removedThisTurn.forEach(r => { if(panel.photos[r.id] != null) delete panel.photos[r.id]; });
      }
    }
...
// Splits the sheet into six switch-view tabs (see #panelTabBar) instead of one long scroll.
...
// Tab 2: Finances + Holdings. Tab 3: Inventory (the Equip compass renders alongside it
// automatically, since it's spliced in right after wherever Inventory ends up — see
// renderPanelHtml below). Tab 4: Skills & Abilities + Learning. Tab 5: Relationships
// (companions will join this tab once that feature exists). Tab 6: Milestones.
function panelTabForCategory(name){
  const n = String(name || '').trim();
  if(/^identity/i.test(n) || /^timeline$/i.test(n) || /^status$/i.test(n) || SCHED_CAT_RE.test(n)) return 1;
  if(/^finance/i.test(n) || /^holdings?/i.test(n)) return 2;
  if(/^inventory/i.test(n)) return 3;
  if(/^skills?(\s|&|$)/i.test(n) || /^learning$/i.test(n)) return 4;
  if(/^relationships?$/i.test(n)) return 5;
  if(/^milestones?$/i.test(n)) return 6;
  return 6;
}
...
function renderPanelHtml(panel, opts){
        ...
        ? `<div class="${chipsClass}">${cat.data.map((it, i)=>{
            ...
            const chipHtml = isInventoryCat
              ...
              : (mergeActive
                ...
                : (isSkillsCat || isSchedCat)
                  // Skills & Abilities and Scheduled Events both get their own always-on
                  // delete cross (not tucked behind a long-press action sheet like
                  // Inventory) — reuses the exact .panel-chip[data-id] / .panel-chip-del
                  // look the dedicated Inventory merge page already defines, just wired to
                  // a plain delete instead of a drag. Scheduled Events entries used to be
                  // permanent/unremovable by design (see the guardScheduledEvents comments
                  // in script_tase.js and the old CAT_INFO "wont" text) — that only ever
                  // blocked the STORY from clearing one on its own; the player deleting an
                  // entry themselves, on purpose, is a separate, intentionally-added path,
                  // wired to data-sched-del below instead of data-skill-del so the two
                  // delete flows (and their distinct confirm copy/target function) stay
                  // unambiguous even though they render identically.
                  ? `<span class="panel-chip${isPast ? ' is-past' : ''}" data-id="${escapeHtml(itemId)}"><span class="panel-chip-label">${labelHtml}</span><button type="button" class="panel-chip-del" data-${isSchedCat ? 'sched' : 'skill'}-del="${escapeHtml(itemId)}" title="Remove">✕</button></span>`
  ...
  // section it summarizes — so on the Inventory tab the order reads the full Inventory list,
  // then this. Skipped on the dedicated drag-to-merge Inventory-only page (opts.enableMerge) and anywhere the section title itself is hidden, since there
  ...
  if(!opts.enableMerge && !opts.hideSectionTitle){
    ...
    if(invIdx !== -1){
      ...
      const equipHtml = renderPanelSection('Equip', renderEquipCompassHtml(invCatForEquip.data, invCatForEquip.ids, panel.photos, panel.equipHub));
...
const CAT_INFO = [
  ...
  { test:/^holdings?/i, title:'Holdings', what:'Real property the character owns \u2014 land, houses, shops, any other fixed property. This section is permanent \u2014 it\u2019s always on the sheet, even with nothing listed yet.', how:'Entirely from the world\u2019s own setup text: whatever property is explicitly stated there gets pulled in once, the moment the world is first created.', wont:'Anything the story itself narrates \u2014 buying a house, inheriting land, opening a shop mid-story never touches this section, no matter how clearly it\u2019s shown happening on-page. This is the one section the story can never add to, change, or take away from; it only ever reflects what the setup text said at the very start. Editing the setup text later doesn\u2019t update it either \u2014 only a brand-new world reads it in.' },
...
// Named (rather than an inline arrow passed straight to one addEventListener) so the exact
// same delegated click handling — skill/scheduled-event delete, the Inventory expand button,
// the "show more" toggle, relationship-name taps, section-title info — works identically
// whichever sheet the click happened in: the main six-tab Letter of Records (#panelContent)
// or the two-page Finance & Gear quick view (#econContent), since both render through the
// same renderPanelHtml and can show the same buttons (e.g. Inventory's expand button).
function handlePanelContentClick(e){
  ...
  // Scheduled Events per-chip delete — same shape as the Skills & Abilities one right above
  // (matched by permanent hidden id, confirm first, save + repaint after), just targeting
  // deleteScheduledEventById (script_tase.js) and its own category instead. Checked right
  // after the skill one so it also never falls through to the section-title info modal.
  const schedDelBtn = e.target.closest('[data-sched-del]');
  if(schedDelBtn){
    e.stopPropagation();
    const schedId = schedDelBtn.getAttribute('data-sched-del');
    if(schedId && state.chattingId && confirm('Remove this scheduled event? This can\'t be undone.')){
      (async ()=>{
        const panel = await getPanel(state.chattingId);
        if(deleteScheduledEventById(panel, schedId)){
          await savePanel(state.chattingId, panel);
          await paintPanel();
        }
      })();
...
els.panelContent.addEventListener('click', handlePanelContentClick);
els.econContent.addEventListener('click', handlePanelContentClick);
