...
// Tab 2: Finances + Holdings + Inventory. Tab 3: Skills & Abilities + Learning. Tab 4 is the
// catch-all — Relationships and Milestones (the fixed category list no longer allows anything
// else to exist).
function panelTabForCategory(name){
  const n = String(name || '').trim();
  if(/^identity/i.test(n) || /^timeline$/i.test(n) || /^status$/i.test(n) || SCHED_CAT_RE.test(n)) return 1;
  if(/^finance/i.test(n) || /^holdings?/i.test(n) || /^inventory/i.test(n)) return 2;
